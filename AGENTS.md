...
## Codex Team 工作流（cmux Codex-teams，2026-07-17 实战定型）
...
写 skill/agent 的 `.md` prompt 时（融合 Anthropic 官方 [skill best practices](https://platform.Codex.com/docs/en/agents-and-tools/agent-skills/best-practices) + 本项目实证；官方 meta-skill `skill-creator` 在 github.com/anthropics/skills）：
...
- **恒真/恒假断言族必崩门**（2026-07-06，588691 三轮取证驱动，全部从 mirror 源码语义推导）：框架 `found/not_found` 是 `re.DOTALL` **无 MULTILINE**、窗口=命令回显+数据+提示符（`send(cmd)+read_until(prompt)`）——由此机械成立七门进 `check_crash_gates_mandatory`：①`^`/`\A`/`$`/`\Z` 锚必假（found 恒 fail、not_found 恒真假 PASS）；②断言 pattern 在被测窗口来源步的**命令原文**上命中=恒真/恒 fail（588691 round1 卷面即「found 恒真+not_found 恒 fail」双恒形态，三轮修法从未触及）；③零 check_point 卷恒 FAIL（`close()` 对 `success==0` 判 fail）；④I 注入 format 结构坏（裸大括号/命名占位/多占位）崩整卷；⑤H 撞框架执行器名字空间（ast 解析 mirror test_xlsx.py 闭集）；⑥cmd_config 含换行被执行层拍平粘连；⑦execute 动作名非 dic_operation 动作注册表精确成员（`command_function_mapping ∪ synonyms(apv+client)`，从 apv_action/client_action + apv_synonyms/client_synonyms 机械解析）——精确匹配失败会走 `get_similar_function` ≥0.8 模糊回退、可静默派发到语义反义 func（#51-A 离线模拟量化 9 碰撞对 / 5 反转对，如 health UP⇄DOWN 0.812、AXFR⇄IXFR 0.944），故 emit 期拒非精确成员 + 最近似候选 hint、绝不 auto-rewrite（#56，设计锚 DESIGN §15 S6 旁）。capture 引用门同步从 lint 前移进必崩门。存量反扫 325 卷：三域当前 pass 单卷零命中，归档区 3 个历史 PASS 卷携恒真断言（当年 PASS 是假验证）。
