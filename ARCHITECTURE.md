...
本文档是 InfoTest Engine 的**单一事实源**，覆盖：端到端数据流、RAG 6 节点流程、模块职责、Qdrant payload schema、环境变量与速查命令。

## 0. 命名与兼容层

- 项目展示名：**InfoTest Engine**。
- Agent 核心展示名：**IST-Core**。
- 当前代码路径、graph id、工具名前缀、collection 名保留历史兼容值，避免破坏已入库数据和运维脚本：`main.qa_agent`、`qa_agent`、`qa_*`、`ultra_agent_vectors`、`ultra_agent_qa`。
- 文档中出现 `main_agent` 时，通常指 IST-Core 的历史代码入口；出现 `qa_agent` 时，通常指兼容包名或 LangGraph graph id。
...
| `qwen` | LangChain `ChatTongyi` 封装 |
| `vector_store` | Qdrant VectorStore 工厂（``langchain_create_qdrant_vector_store``） |
...
在项目根目录 **`environment`** 文件中配置（不带 `.env` 前缀；无 `.env` 扩展名）：
...
QDRANT_PORT=6333
# QDRANT_COLLECTION_NAME=ultra_agent_vectors   # 可选
# QDRANT_QA_COLLECTION=ultra_agent_qa          # 可选
```
...
```
main/qa_agent/            # 顶层 StateGraph + IST-Core / 专用 Reviewer 兼容实现（deepagents）+ Tool + Sink
...
tests/qa_agent/            # smoke + event_contract + eval + 3 datasets
...
## 12. v1.6 对话式评审闭环（InfoTest Chat UI + IST-Core 编排）
...
flowchart LR
    User[用户在浏览器]
    UI[InfoTest Chat UI<br/>agent-chat-ui<br/>Next.js 15<br/>:3000]
    API["langgraph dev server<br/>:2024<br/>(Windows 下必须 --host 0.0.0.0)"]
    Graph["main.qa_agent.graph<br/>normalize_input → qa_node / review_node → finalize"]
...
### 12.4 专用 Reviewer（`main/qa_agent/agents/reviewer_agent.py`）
...
**Prompt 工程化**（R1-R2）：
- `main/qa_agent/agents/_prompt_assembler.py` 新增 `SkillAssembler`：从 `skills/<Skill>/fragments/*.md + manifest.yaml` 按 case 条件装配 system_prompt，替代原整 markdown 字符串拼接
...
**结构化预分析仍保留**（不是 enforcer 改写，是 prompt 注入）：
- `main/qa_agent/agents/pre_analysis_middleware.py:PreAnalysisInjectionMiddleware` 在 sub_agent system_message 末尾注入 D6/D7/D10 程序化预分析事实（baseline 命中矩阵 / requirements / dependencies），让 LLM 直接基于字段推理
- 注入是 ground truth 提示，不改 LLM 输出；最终 finding 仍由 LLM 写
...
### 12.5 LLM 工厂关键决策（`main/qa_agent/agents/_llm.py`）

| 环境变量 | 行为 |
|---|---|
| `QA_AGENT_FALLBACK_MODEL=openai:qwen-plus` **（推荐）** | `ChatOpenAI` + `base_url=DASHSCOPE_BASE_URL`，走 DashScope OpenAI 兼容端点 |
| `QA_AGENT_FALLBACK_MODEL` 空 / 包装失败 | 降级到 `ChatTongyi(model="qwen-plus", streaming=False)` |

> **为什么 openai 优先**：`langchain_community.chat_models.tongyi.ChatTongyi` 在 `streaming=True` + `tool_calls` 情况下有 `subtract_client_response` IndexError 越界 bug（v0.3.x 未修）。走 ChatOpenAI + DashScope 兼容端点完全绕开，tool_call 稳定。
...
### 12.7 运维与启动（PowerShell 控制脚本）

```powershell
# 启 Qdrant（容器，一次性）
podman run -d --name qdrant_ultra -p 6333:6333 ... docker.m.daocloud.io/qdrant/qdrant:latest

# 启后端 (LangGraph dev server, :2024) + 前端 (agent-chat-ui, :3000)
.\tests\qa_agent_backend.ps1 -Action start
.\tests\qa_agent_ui.ps1       -Action start

# 查状态 / 停止
.\tests\qa_agent_backend.ps1 -Action status
.\tests\qa_agent_ui.ps1       -Action stop

# 浏览器打开 http://localhost:3000
```

**Windows 幽灵 socket 规避**：`qa_agent_backend.ps1` 默认用 `--host 0.0.0.0`
（Windows 有时会出现 `127.0.0.1:2024` 被已退出进程遗留的 kernel socket 占用，
改绑 `0.0.0.0` 可绕开）。agent-chat-ui 通过 `http://localhost:2024` 仍能连。
...
| ChatTongyi streaming + tool_calls IndexError | `_llm.py` 默认 `streaming=False` + 优先走 OpenAI 兼容 |
...
最小端到端命令（`environment` 已配 DASHSCOPE_API_KEY + QDRANT_HOST 后）：
...
python -m main.qa_agent.runner --task qa "如何配置 HTTP/2 SLB？"

# 3) 专用评审场景（结构化 JSON 输入）
python -m main.qa_agent.runner --task review --input tests/qa_agent/sample_review_bug_fix.json

# 4) Streaming + 事件总线
python -m main.qa_agent.runner --task qa "..." --stream --verbose

# 5) CI 评估门禁（跑 ta-qa-v1 硬阈值）
python -m tests.qa_agent.eval --ci
...
#   入口配置：根目录 langgraph.json → main/qa_agent/server_graph.py:graph
#   server_graph 用 build_qa_agent_graph(checkpointer=False, store=False)，
#   持久化由 langgraph-api 平台注入。

# 8) InfoTest Chat UI（Next.js 15，pnpm；连本地 langgraph dev 直接对话 IST-Core）
cd agent-chat-ui && pnpm install
cd agent-chat-ui && pnpm dev          # http://localhost:3000
#   .env 配置 NEXT_PUBLIC_API_URL=http://localhost:2024 + NEXT_PUBLIC_ASSISTANT_ID=qa_agent
```
