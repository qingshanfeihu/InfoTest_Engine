...
```bash
MINERU_TOKEN=xxx
DASHSCOPE_API_KEY=xxx                  # 或 DEEPSEEK_API_KEY，二选一
IST_LLM_PROVIDER=dashscope             # 或 deepseek
IST_MODEL=qwen-plus
IST_SQLITE_PATH=./qa_agent.sqlite
```
...
按 `IST_LLM_PROVIDER` 路由，统一通过 ChatOpenAI 兼容端点（共享 `ChatOpenAIWithReasoning` 子类，处理 `reasoning_content` 双向 patch）。

| `IST_LLM_PROVIDER` | base_url | api_key | extra_body |
|---|---|---|---|
| `dashscope`（默认） | `DASHSCOPE_BASE_URL`（默认 OpenAI 兼容端点） | `DASHSCOPE_API_KEY` / `BAILIAN_API_KEY` / `OPENAI_API_KEY` | `enable_thinking=True` |
| `deepseek` | `DEEPSEEK_BASE_URL`（默认 `https://api.deepseek.com`） | `DEEPSEEK_API_KEY`（必需） | `reasoning_effort=max` |

> **为什么放弃 ChatTongyi 原生 SDK**：`langchain_community.chat_models.tongyi.ChatTongyi` 在 `streaming=True` + `tool_calls` 情况下有 `subtract_client_response` IndexError 越界 bug（v0.3.x 未修）。统一走 ChatOpenAI + DashScope 兼容端点完全绕开，tool_call 稳定。Anthropic 兼容路径已下线（百炼 anthropic 协议存在 usage metadata 兼容错误）。
