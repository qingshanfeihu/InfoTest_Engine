...
| PASS 即时写回（writeback_one） | latent 消除（可达性维度） | ✅ | 608 selfheal1 死/selfheal2 过的因果案例 |
...
| ledger 迁移合法性表（passed 终态锁） | 状态一致性（非 K 状态机，是 case 状态机） | ✅ | 与 K 状态机并列的第二状态机——见信号机统一记录 |
| loop_guard / tool_result_prune / 摘要中间件 | 理论外的资源护栏 | ✅保留 | 不属三合取/K 理论管辖，工程必要；标注"理论外"防止误纳入理论叙事 |
| frozen 闸 + override 通道 | 行为层"已证伪子空间"的签名级近似 | ✅ | 状态机 frozen 语义（换法标记非终态）；与单调门组合后 override 不再能删覆盖 |
...
1. 信号机（§三，其余各项的记录基座）→ 2. 单调门 → 3. build 锚三源 → 4. 体检扫描器+quarantine → 5. 被拒命令∉意图信号 → 6. 死代码清理 → 7. dream 约束。
