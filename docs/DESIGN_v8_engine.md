...
| 执行位阶律（公式47，2026-07-16 理论新增） | 门体系四位阶分解：语义门（禁）vs 结构门（留）二分——全 74 门的收录/退役判据（详见 `DESIGN_dongkl_finalization.md §0.1`，2026-07-17 回填本表） | grade 之死即「模型判断单独当质量门」的反例 |
| 辖区判据（公式18b，正本 `THEORY §5.5.6`） | 数据归 INV-7「业务态以事实流为准」还是「门挂独立凭证路」取决于**角色**：业务态（counts/verdict/deliverable）须 facts 单源防显示漂移；防伪凭证须独立于被检查方防自证。**精确判据＝门不能读被检查方在凭证信号（interrupt resume）之前能自达写入的账**（写点 post-signal 即抗伪；凭证可进 facts——抗伪不来自"在流外"、来自写点时序位置）。folding 门变体 A 首个实证（正本 `THEORY §5.5.6`、设计投影 §18.16） | 旧「门不能读引擎写的账」过粗——引擎 resume 之后才能写的合法凭证被误禁 |
...
```
（现役图,2026-07-17 按 graph.py 重画——初版图见本节末历史注;11 节点与 NODE_TYPES 一致,拓扑门三方断言）

prep ──▶ bed_gate ──▶ author ──▶ merge ──▶ run ──▶ reconcile ─┬▶ attribute ──▶ diagnose ─┬▶ reflow → author
[mech]   [mech+user]   [LLM孔]    [mech]    [mech]    [mech]    │    [LLM孔]     [mech]    └▶ rerun/瞬态 → merge
              │            │                            ▲       │                             (s₀ 案复跑闸挡)
              │            │ 欠定/存在性未命中:           │       ├▶ 待终验(子集过)/broken 复跑 ──▶ merge
              │            ▼ 该案 suspended(不产卷)      │       │   (终验=ctx=delivery 的 run,同一 reconcile 入账,非独立节点)
              │     其余案继续跑;全批无可推进工作时 ──────┘       └▶ 矛盾≥2/env 待确认 ──▶ ask_contradiction [user孔]
              │            ▼                                          （封顶/挂起恢复亦经 author 路由至此;
              │     ask_decision(批末 gather) [user孔]                  答题→可推进案回 attribute/merge,零实答→收口）
              ▼
        bed_blocked → closing [mech]（writeback/report=closing 内动作;provisional/确认/rollback 三段事实照旧）
```

图纪律：条件边全部是 state 计数缓存的纯函数（`graph.py::_after_*`，边级注释即权威语义）；
所有 pre-ask 收口边经 `_gather_or_close` 前置门（有欠定必先呈报，§18.2 式③不静默）。
fail 后半场（suspended/gather/diagnose/remedies）语义详见 §16.1；ask 三态见 §16.1「问询语义三态」。

> 历史注：初版图（2026-07-10）为 `prep→anchor_gate→author→ask(欠定阻塞)→merge→run→reconcile→
> attribute→route` + `final_run(ctx=delivery)→writeback→report` 独立链——anchor_gate 后升格
> bed_gate（版本只是床态一维，见下）、欠定 ask 由阻塞改 suspended+批末 gather（§16.1，R4 裁决）、
> route 并入 diagnose 路由、final_run/writeback/report 收敛为 run 参数与 closing 内动作。
...
- **双账职责声明（对抗审查）**：`verified_runs.jsonl` 保留为**全局跑账**（跨批、按 autoid，
  ...
  **批状态机**。两账由同一 digest 写入点同时追加，语义上事实流为批内真理、verified_runs
  为跨批凭证索引。
- **run 节点失败语义两件（2026-07-16 zhaiyq 续跑两轮实证回填，commit 14c12163；
  INV-11 式②的可诊断性侧）**：① **digest 留声**——run 失败时 emit+error 必须携带
  digest 返回串头 300 字（实证形态：digest 秒回无 last_run 时 error 只有七个字，
  deliver 失败/探测拒绝的真实原因全被吞，盲修两轮）——失败语义不仅是「不静默」，
  还要**带原话**（同 §18.10「衍生视图与被审窗口共享失真」的教训：诊断依赖一手证据）；
  ② **stale_run_on_device 归 busy 语义**——跳板机残留 pytest（被打断批）探测命中
  从 error 路归入 `device_busy`：残留是「床被占」不是「引擎错」，busy 语义走上机
  互斥的既有处置（拒跑/确认弃跑后 force_clean），不进 error 收口路径。
...
- 保存族编写建议（对抗审查，C 层）：持久化家族 case 建议**案内自清**（案首/案尾清理自己
  ...
  ~~机制替代（clear+reload 代真重启）须在 D 列声明~~——**2026-07-14 F7 撤销**:该条款
  正是写保存族 σ 链断裂的引导源(desc 声明无人消费+范例被 100% 套用),替代一律走
  F6 禁令机制路由(§18.11:要点先行+等价判据四条+用户裁决),不再有"声明即可"通道。
...
- 坑叙事的正确归宿是**判例层**（footprint 观察/策展标注，运行时检索），不是 prompt 常驻
  （自愈合架构本来就为此而建）。
- **治理（⑥C，用户 2026-07-19 裁定 ③：废行数预算 → 归属行 + 熔断）**：**废除固定行数预算**——旧 worker≤60/attributor≤50/SKILL≤80 及历史 99/86/206 等数字一并作废、勿再引。改为两条约束：
  1. **归属行（存在性覆盖率，非行数）**：每条存留规则必须在 `references/theory-map.md` 有归属行（理论构件或现行数据点）——**无归属=该删**。规则的正当性挂"有没有理论/数据据"，不挂"占多少行"。
  2. **150 行熔断线**：单文件超 150 行时，**先置换（删无归属/过时规则）再增补，不得净增**——熔断不是硬上限（有据的规则可越线），是"越线先做减法"的强制触发（防 §18.13/§18.14 那种多轮增补不回填的静默漂移复发）。
  删除入 `references/removed-rules.md`（一行一条+旧归属）；失效模式复现按 §5.5-③（现行体制下实测复发）以数据为据恢复。**账上既执行一笔置换**（随批）：检索顺序细节从 prompt 移入工具说明（`contracts.md` 机读契约面）——prompt 只留机制、数据按引用（同 reference-docs-mechanism-not-data）。
...
main/ist_core/skills/ist-compile-engine/     # V8（user-invocable）
  SKILL.md                                   # 主 agent 面:什么/何时/怎么调/怎么转述——行数纪律见 §5.5 ⑥C
  references/
...
| 官方规范条目 | V8 落点 |
|---|---|
| Progressive disclosure 三级 | L1 frontmatter（name/description/when_to_use）常驻；L2 SKILL.md 体只服务主 agent（行数=§5.5 ⑥C 归属行+150 熔断，**废固定行数预算**）；L3 references/ 按需（contracts 只被孔/评审读） |
| Concise is key | §5.5 减法纪律：三选一存在性检验+门在则不教+坑叙事归判例层；**勿再引**旧 worker≤60/SKILL≤120 数字 |
...
**（2026-07-17 补指针——本表两处已被后订条款收窄,按表施工前先读）**：
① 通道①缓解「merge 排卷尾」经 run11 实证**对跨轮通路无效**（快照跨轮存活,排尾只降
卷内暴露——668030 排尾后仍翻挂,§16.4 片3 定形②）,持久面毒源的 fix_direction 已
**删排尾路线**（§18.8 校准③）;排尾仅存卷内暴露缓解价值。② 本表动词族判据已被
§18.12 数据驱动收窄:`config memory/file/net/all` 恢复类是**读**非污染源（50% 误判）、
`write memory` 框架 clear+逐命令逆操作**清得掉**（11%）、`write file/all file` 仅在
**跨案撞名**恢复时构成真污染路径——s₀「持久面写」判定以 §18.12 修法为准,本表保留
作通道形态与 mitigation 字段的数据契约说明。
...
2. **需要 LLM 产的人话，在判断时刻一次性落成事实字段**。归因孔新增三个结构化字段（见 11.3），渲染层只取用不再生成。符合语言分层：`fix_direction` 保持英文（喂下轮 brief 的 LLM 面），`user_note` 中文（交付物面）——同一事实按受众双通道。**空值兜底（D28 基准#4）**：user_note 空时（旧记录/某轮未产）渲染层退**中文归因类目**（`_DISP_CN`，如「其他判断」）而非 fix_direction 英文——否则英文散文直灌用户面=D1/D9 泄漏；即 ⓑ主（user_note 中文叙述、含 545249 实证的失败趋势）+ⓐ兜底（类目中文），主路径与空值兜底皆不露英文（消费点 `_case_diag`/`claim_history`/cap evidence 同此口径，`_NARRATIVE_FIELDS` 中文占比门守）。D28 复用既有休眠字段 `user_note`（消费点+契约早成型、只差写入）而非新造 `user_summary`——framework-capability-before-limitation。
...
**选项文案契约（D2/D4 定案回填 2026-07-18，后果人话推广至全部面板）**：ask 缺陷单实证——选项恒为引擎内部动作名（欠定「改过程/改预期/改描述」、矛盾「重排复验/如实降级」、cap「继续/挂起/停止」），无一附「这对你的用例意味什么」，普通测试用户无 V8 引擎知识无法知情决策（**全 43 面板系统缺陷，D2 定级最严重**）。「每个选项的后果一句话」不止矛盾题面、**推广到欠定/矛盾/cap 三类全部面板**。落地靠 **label/description 双通道分工（承重面不同，不可混写）**：
- **`label` = 解析 token 承重、保持稳定**：`_bridge` 每题携带 `label→token` 同源精确映射（W3 修复依赖 label 稳定，label 是 `"{header}"="{answer}"` 的解析锚）——**后果人话绝不塞 label**（塞了破 `_answer_token` 精确映射）。label 用简洁固定人话模板（D4：不把等价方案/命令原文前 N 字符硬截断进 label——曾出「采纳「…不带 query_type 执行 `no sdns session persist」」半句截断）。
- **`description` = 后果人话「对你的用例意味什么」**：无 V8 知识可判的后果说明、等价方案技术细节放 description/题面区，**不放 label**。选项文案按「用户意图」表述（如「这个用例的验证步骤不够、请引擎补足」而非内部动作名「改过程」）。

契约边界：仍走既有 ask_user 契约（≤4 题/批、解析协议不变），改的是 label 人话化 + description 补后果，**不改组件/解析协议**——纯文案层、零机制新增。
...
（不符脑图交付预期）、「报缺陷」被五查机械排除（海量回显是手册内行为且正是被测对象）——菜单
...
- **选项级机械门**：缺陷选项仅当五查通过（行为与手册矛盾）才允许出现；止损选项仅当导出修法
  已试尽或用户主动叫停时出现。
...
### 11.11 Ask 子系统终版设计（2026-07-10 定稿 = THEORY §2.6 参数集 × 调研四定案）

> 引用锚校注（2026-07-17）:「十参数」是 §2.6 定稿时点的参数清点口径,理论四稿改版后
> 现行文本无此词——现指 §2.6 本体四件(分界/权威/收敛律/采信)+ §2.6.5 对抗补出的
> 参数族(意图触碰面/应然锚/判例键/cap 二分/代理假设/检索触发/采信条件/改写-填充精化)。
...
### 11.12 leak_scan 术语泄漏兜底 detector（原则3机械落地，F-Py-2/3 定案回填 2026-07-18）

**定位**：§11.1 原则3「对外零术语泄漏」的机械守门。术语→人话是 scrubber（词表主动映射）主线；leak_scan 是**兜底 detector**——scrubber 漏网的机读锚在交付前被抓/兜底替换。**两层分立**：scrubber 主动翻译（术语→人话词表）、leak_scan 被动检测（漏网锚 flag/替换）——**detector 加宽 ≠ scrubber 加词**，加检测面不动翻译纪律。用户观察员实证泄漏面：`nd:`/`facts.jsonl`/`escalated(_pid)`/18位 autoid/deliverable 英文枚举/内联文件名。

**双 detector**：
- **token 类级 detector（F-Py-3）**：抓 snake_case 机读枚举 / `nd:` 前缀 / 18位纯数字 autoid / `.jsonl` 类扩展名——**token-shape 结构锚**（要求下划线等形态特征），非强字典关键字白名单（`[[compile-judgment-structural-not-strongdict]]` 同型）。**假阳控制**：要求下划线排普通英文缩略语，路径段 / 扩展名 / 常见技术词豁免——低假阳。按 token **类**（形态族）判、不逐枚举硬编，新枚举同族自动覆盖（自愈合纪律：加检测族零代码 / 加数据）。
- **中文占比 detector（F-Py-2，`chinese_ratio`）**：叙述字段（user_note/hypothesis 等人话面）中文占比过低=英文机读锚渗入叙述——补 token detector 未穷举的泄漏（叙述该是人话）；仅对 `len(strip)>=8` 的实义串判（短串免噪）。

**作用域三分（防误伤，F-Py-2 核心设计）**：
1. **叙述字段验**（user_note/hypothesis/remedy_text 等渲染出的人话面）——两 detector 的作用域，该是人话、英文锚=泄漏。
2. **原文引用豁免**（`device_quote`/`doc_quote`/`quote`）——设备回显/手册原文本就是英文技术内容，如实转述非泄漏，豁免中文占比 + token detector（引号内是证据、不是渲染叙述）。
3. **机读码不进作用域**（fact 顶层）——detector 只吃渲染出的 `claims`/叙述串，**不传 fact 顶层机读码**（autoid/facts.jsonl 指针等本就该在事实流、不入用户叙述面，从源头不进作用域，非事后豁免）。

**准则**：detector 是渲染兜底最后防线（scrubber 主线漏网兜底），非替代 scrubber；作用域三分保证不误伤原文引用/机读码；结构锚+豁免控假阳（eval-first 验：路径段/裸命令假阳都经 eval 逼出后收窄判据）。**违 reason=WHY 叙述契约的 flag 保留**（裸命令 reason 触 flag=坐实器暴露边界让人裁，非证伪判据——判据不因个例松）。
...
| I2 | #67 归属一致性门（§7 通用形态） | **已落防御侧（2026-07-14+；§15.3 队列项 2）**：`nodes._evidence_suspect` 在 attribute 收证时对取证附件跑「执行目标 vs 卷面 G 列」机械比对；不一致→`evidence_suspect` 事实+brief 声明「取证疑似错位，勿采信」 | 防御侧已接线；切分 bug 本体（框架/digest 侧错位根因）仍待修——勿把「防御侧已落」写成「#67 全闭」 |
...
**S6 姊妹门 · execute 动作名精确集门（#56，emit A 层 membership 门，2026-07-19；#51-A 离线模拟量化驱动；随 #56 四关批 land，代码由 Py-Eng）**：与 S6 命令存在性门同族（emit 期查名 ∈ mirror 解析闭集），治 execute 派发的静默语义反转。
- **门**：`F=execute` 的 G 动作名（全角 `：` 前段）须精确命中 `dic_operation` 动作注册表——闭集 = `command_function_mapping`（`apv_action.py:11-44` ~33 + `client_action.py` ~8）**∪ synonyms(apv+client)**（`apv/apv_synonyms` + `client_synonyms`，client 动作如 `访问` 依赖后者），即 `get_same` 能精确匹配的集合，**从源码机械解析、版本闭合**（同 ⑤H 名字空间门 ast 解析 test_xlsx.py 闭集；CLAUDE.md 必崩门 ⑦）。**synonyms 必纳入精确集**（binding spec）——否则误杀合法 synonym 命名的金标准卷（比无门更坏）。
- **起因**：`dic_operation.execute:57` 派发 = `get_same` 精确失败 → `get_similar_function` **模糊回退（SequenceMatcher ratio ≥0.8）** → 全不中才 None。#51-A 离线模拟量化 40 动作名间 **9 碰撞对、5 组语义反转**（health-wait UP⇄DOWN 0.812 / bind⇄check 0.897 / AXFR⇄IXFR 0.944…）：worker 写近似名 + 精确失败 → 静默派发到语义相反 func、**设备级不可见**——比 §6-S2「误派 None」更毒（是「误派反义」=潜在静默假验证，严重度同 S6/⑤H 门族）。
- **门形态**：G 动作名 ∉ 精确集 → 拒落卷 + 最近似候选 hint（列注册表最近的名），**绝不 auto-rewrite**。
- **§21 推导**：需 gate 性高（静默语义反转=高错误代价 × worker fork 无人值守）× A 层可 gate 性（精确成员是机械可判定闭集）→ **写时机械门（强制单路）**（§21：门密度 key 需 gate 性、门形态 by 可 gate 性）；**hint-not-rewrite** 因 auto-correction 会把危险的 ≥0.8 模糊匹配**机械化重现**（自动改最近似 = 正是那个反转 hazard）——hint 只授权「这有个近似名」由 worker 定、不授权「这就是对的名」（§21 escape-hatch 有摩擦；相似 ≠ 同一）。
- **操作化 §13 I5:666**：I5「0.8 模糊匹配契约」flag 的正是此 hazard；本门 + `tests/framework_contracts/` 模糊匹配契约测试落地它。**retro-scan/tests 须覆盖 synonym 命名行**（金标准可能用 synonym 形态）。
...
2026-07-17 结构修复——本段原插在表格中间致片4 行脱落，移至表后）**：
...
delivery 裁决在案∧组成内无 subset_verified 待升格案∧**组成内无 broken 三态案**
→ 不重跑（livelock 防护；瞬态恢复的升格终验放行——redline 实证回归。
**broken 断批例外，2026-07-16 zhaiyq 续跑实证补**：被杀 run 的断批 delivery 裁决
（17 过+27 not_run）带同卷指纹入流，续跑曾被幂等闸误吸收→27 真通过案钉死 broken
→delivery_incomplete 收口——闸的设计意图是吸收**零信息重跑**，断批重跑非零信息；
判定抽纯函数 `_delivery_verify_skippable`（可测），既有 subset_verified 待升格
例外语义不动。测试锚：`test_delivery_idempotency_gate.py`，commit b3ce3b4b）。**①A 缝合扩展（2026-07-19 用户裁，THEORY (15) 补注·缝合语境等价）**：闸另增两条件否决——同床同版本（cur_bed/cur_build 一变即非同一 ctx_delivery）＋断点两侧独立（coexist 非空=保存族反例 δ(c)≠∅、拼接退化 ∃-pass 失真），任一破 ⇒ 回整卷连跑重验。**续跑代价注**：coexist 非空卷续跑不幂等（每次续跑整卷重验）——条款⒉安全侧取舍，宁多跑不 ∃-pass 假交付（coexist 静态形态检测保守、可能误报；单案残留假 PASS 属 oracle 残差职责、缝合不新增——Theory 协审边界）；
⑤附带根治既有地雷：merged 事实
...
### 16.5b dongkl 定稿回指（2026-07-17 补,治「§16 缺跨文档回指」）

本章 fail 后半场的两族后续修复定稿在 `DESIGN_dongkl_finalization.md`,主文档在此回指:
**§⑥ ask-liveness 修复**（broken 连击 per-case 护栏、卷指纹隔离、gather 活性——代码锚
nodes.py/views.py 的对应注释以彼处为设计权威）;**§④ broken 三态 pyATS 七码**（§18.1
的采集子类定形）。彼文档 §5 为 team3 增补设计（A1/A2/B1'/D9/C20）,同属本章后续。
...
> **队列版本注（rubric/D 评审回填）**：「v4」= §15.3 队列 v3 上 **G1–G6/C1 片** 的落地增量
> 记名，**无独立「队列 v4」章节**。执行队列唯一权威仍是 §15.3 队列 v3；INFRA §9 指针同此。
...
- ~~**恢复类命令泄漏清理**（缺陷⑥第一版,数据驱动）:domain_grammar 新增
  `restore_leak_teardown`……~~ **已被 §18.4.1 整条删除**（2026-07-13 用户裁决：经验命令
  注入致两床被跑死）——勿按本段作现役设计；668000 现象改走判例层 uncertain 观察。
  残余课题（pass 案 quarantine 入口/框架 per-case clear 加强）仍在 #74 登记。
- **skill 禁旁路**（缺陷④）:ist-compile-engine SKILL.md 增引擎异常处置约束
  （只能同参续跑或如实上报;禁手动合并/手动上机/直改 xlsx——绕门断审计链,
  run13 实证直改致凭证失效全批停摆）。

### 17.5 G5 二犯根治：escalated 解除同口径 + 等价守门（2026-07-18，D31）

- **漂移根因**：report_gate 独立重算（故意不复用 views，防 bug 传染）曾用无条件 `any(escalated)` 排除，与 case_status 的 run18 解除语义（最后 escalated 后有 authored 即解除、迟到落盘凭证有效）分叉 → zhaiyq 516389/533097 迟到回收卷被误判不支撑、G5 自我误告警（42 真值判成 40）。
- **修法**：report_gate 的 escalated 排除口径 inline 镜像 `views._is_escalated`（`last_esc≥0 ∧ last_auth<last_esc` 逐字等价、与 suspended/resumed 解除同型）——两路各自实现同一解除语义、门独立性保留。
- **等价守门（理论正本=THEORY §5.5.8 独立重算路径逐点等价律，此处零双写引用；选择判据「门独立性 required→双路+守门」亦见该节）**：新增 `test_report_gate.py::test_recount_matches_case_status_all_lifecycle_branches_D31`——合成事实流穷举全状态转换枝（escalated 解除/终态、suspended 解除/终态、终局 round99、二次欠定 qid 配对、未答呈报、clean），**单测期**断言 recount 与 case_status 逐案等价 + **双重断言**（两路等价 ∧ ground-truth，堵"两路同错都绿"）。守门完备性纪律入测试 docstring：新增排除态必同 commit 加对应 fixture 枝（二犯真因=fixture 缺 escalated 解除枝、机制在 pytest 照绿）。
- **REPORT_MISMATCH 自降级 UX 条款（诚实边界）**：门触发失配时，报告必须①**自标不可信**（"校验未通过、本批暂不可作为交付依据"）②**指向机读源**（以 `facts.jsonl` 与 `REPORT_MISMATCH.json` 为准）——判定式渲染产物在双路校验不一致时数字不可信，自降级+指向机读源是正确自纠（同"系统自曝不可信是健康的"精神，非物理扣卷）。
- **迟到回收卷 run18 语义适用（516389 定谳，维持真值 42）**：终卷即迟到回收卷（fork 墙钟超时 escalated → late artifact reclaimed=authored 在 escalated 之后）机械满足凭证（`.grade_credential.json` lint_ok+mtime 吻合 → 合法 authored、无 emit_invalid 打回）→ 解除 escalated → deliverable。既定语义、非新政策。
...
### 18.7 完成度纪律
（2026-07-17 物理归位：本节原被 §18.8-18.15 的追加轮挤到章末，编号与位置脱节，现挪回编号位；零重编号）
...
**修订波及面自查（2026-07-17 立,team4 与 Theory 审计独立收敛的同一病灶——追加式
修订只写新条款、不回填旧锚）**:任何「撤销/取代/收窄」条款落笔时,同一提交内 grep
被撤对象名在 docs/ + CLAUDE.md 的全部命中处,逐处三选一——打指针(「已被 §X 修订」)/
回填(汇总表、映射表、落地序同步)/如实豁免(列出不改理由);**取代声明必须双向**:
A 宣称取代 B 的某节,B 该节须有反向指针,且 A 须逐单元交代去向(含「缓立/取消」——
§18.15 单元 E 落空即缺这一句的实证)。
...
### 18.10 oracle 残差门与证据链评审（2026-07-14 窗口失真彻查;框架判定不再是公理）

**实弹**:79 床三个 run(run18-20)全量原始流对账清点——127 个断言中**假 FAIL 7 起**
(对齐响应块含期望值却判 fail)、**假 PASS 12 起**(not_found 判过但对齐块含该值),
100% 集中写保存族;**668000 假 PASS 三连已作为 verified 先例+footprint 写回(投毒)**。
机理:框架 `read_until(prompt)` 读窗在异常回显(`^` 语法拒绝/缺参提示/慢响应)下整体
错位,断言在错误窗口上求值;668015 check1 的失败窗里装的是上一条命令的例行 Warning。
同一失真通道喂养归因/诊断/审计,14 个 run 里每轮都产出自洽的假故事(床污染/再生毒源/
SLB 复活),经用户逼读原始流才破。

**为什么发现不了(六个结构性机制,设计教训)**:①权威序把 oracle 排除在被审对象外
(74 门全防 LLM 说谎、零门防 oracle 说谎);②所有调查消费同一失真通道(自证结构,
同构冗余共因失效);③假 PASS 不呼救而修复循环是症状驱动的(静默类结构性漏掉);
④解释性遮蔽(每轮都有"够用的真解释"吸收残差,跨 run 复发从未作为独立异常升级);
⑤评审全横向(资产×规范)无垂直(单案证据链逐层转换核对);⑥归因无完备性义务
(失败窗里躺着错位铁证 14 个 run,无人要求解释窗内每一行)。

**构件与状态**:

- **窗口出处对账门(已落地)**:digest 对每案(含 pass——假 PASS 不呼救,必须主动审)
  旁路取原始 inner+apv 全文(`fetch_case_raw`,不混流不截选——衍生视图与被审窗口共享
  失真,审计必须异构),按提示符重分段得真实 命令→响应块 映射(`_apv_blocks`),第 k 个
  check 对齐第 k 个响应块,**方向矛盾=采集面失真**→verdict 降 broken(第三态:挡写回/
  挡归因/挡 s₀ 配对)。失败语义:旧 client/替身无 raw 方法→rec 标 `window_audit:
  unavailable`(INV-11 不静默)。范围:源命令可定位于 apv 会话的 check(dig/触发端源
  如实跳过)。测试锚:`test_window_audit.py`(8 项,fixture 取自 run20 真实形态)。
  理论位:K §5.5 oracle 残差公理(缺口 E)的窗口出处面兑现;(π,R) 忠实投影的运行时
  违约探测器。
- **块级 ^ 拒绝检测(已落地,同门内)**:响应块含裸 `^` 行=设备解析拒绝(闭合于设备
  解析器标记,非关键字表)→ 记 `syntax rejected (^): <cmd>` 入 anomaly_lines——G6
  anomaly 否决与 brief `<execution_failures>` 高亮随即生效(本次三案的 s₀ 误判本可被
  此路径机械拦下)。
- **occupancy 文法出处更正(已落地,数据)**:「may already be occupied…please check」
  是 sdns listener 配置的**例行 advisory**(干净床实测:警告出现且配置成功),非占用
  陈述——负向排除;echo_confirmed 只认真实占用陈述。原条目 2026-07-12 的种子例证即
  这句 advisory(失真通道下的误读),出处已更正。
- **投毒清算(已执行)**:verified_668000/668015/668044.xlsx 移出 mirror 检索路径
  (隔离于 runtime/backups/poisoned_precedents_20260714/);run13「config 恢复泄漏
  不可见占用」uncertain 观察×3 撤销(其"恢复后 listener 不出现"即窗口错位产物)。
  **run13 再生毒源叙事连带存疑**——当时判定骑在同一失真通道上,缺陷候选降级待复核。
  cli.commands 的语法接受性证据**不撤**(命令确实执行,失真的是断言判定)。
- **S₀ 前置裁决(原名 G5 前置自动裁决,改名消与 G5 报告重算门的一名两义;
  完整定界见 §18.12——本条初稿的「隔离仍挂=污染假设被反驳」经理论对界**过宽**
  (只对纯卷序分支成立,含持久面毒源时隔离仍挂是不可辨态),裁决表以 §18.12 为准)**。
- **跨 run 复发协议(立项)**:同案同断言跨 run 失败≥3 次 → 历届归因**全体作废**,
  强制原始流垂直取证,禁止产出新的同层解释(准则一「症状反复=根因没找到」的机械对应;
  需跨 run 复发台账,与床账同型 per-host 持久)。
- **垂直取证评审(纪律条款)**:每批 closing 抽一案做全链转换核对(脑图→卷面→原始流
  →窗口→判定→归因→问询);窗口对账门已内建每案机械面,人工垂直评审作为对"门本身"
  的抽查。
- **信任根行为契约(立项,I5 补全)**:登记册内每个信任根配运行时违约探测,不只新鲜度
  锚——框架 read_until 的异常回显契约测试(`tests/framework_contracts/`)为首项。

**V6 考古与跨版本坐实(2026-07-14 补)**:对 103 跳板机 176 个 7 月 report(3400+ 案
日志、12658 断言)跑同一窗口清点——dongkl 家族假 FAIL 68 起/14 案、**假 PASS 4 起/
3 案(105877/105910/681783,其 verified 卷至今在 mirror 被 footprint 引用)**;
yzg@103 假 PASS 21 起/4 案,**正是写保存族 668000/015/030/044**——跨床(103/79)、
跨引擎版本(V6/V8)同型复现,彻底排除"单床/单版本"假设。V6 代码考古:verify_phase
直接采信 digest verdict 且 **PASS 即时写回**(同 run 内投毒传播);V6 自己记录了
dongkl 6/21、zhaiyq 22/50 的 `verdict_flip_on_locked_pass` 审计事件,并以"运行时
欠定/时序 TTL 盲区"解释吸收——机制④(解释性遮蔽)的成文历史实锤。另:V6 attributor
的 prompt 写明五查含 `compile_precedent` 同意图对照,但 tools 白名单从未包含该工具
(指令不可执行=纯缺陷),**该矛盾原样遗传至 V8.5 现役白名单**。

### 18.11 是-应对称与判例分层（五稿=现行规格,2026-07-14;全节**设计中**,已过双路对抗评审）

理论位:K §2.6.6 对称怀疑推论 + K §2.9.4 (45)(45b) + K (40) 第七类 + ask 上游化
(K (35) 意图链)。**版本史**:二稿(σ 链定性+F6/F7/F8 初形)→三稿(配置面模型)→
四稿(山穷水尽方向性,重启→clear)→**五稿=对抗评审合并**(理论路 18 项+设计路 25 项,
2026-07-14):二稿「单判据等价」、三稿「无替代投降」、四稿「模型担保等价」的表述
**均被取代**,以下为唯一现行文本。共 **八个构件**(F1-F8)+两项横切必修。

**横切必修(现役缺陷,独立于任何 F 件)**:
- **H1 「改过程/改预期」落盘死路**:`nodes.py` ask_decision 调 `compile_user_decision`
  不传 assertion_form,工具对非「改描述」无条件要求 form → 决策落盘必败、下轮重问
  (问询活锁;655248 走通纯因选了改描述;e2e 未抓因 rig 打桩)。修:按 claim_kind
  条件化 form 要求(forbidden_mechanism/mechanism 类免 form,等价实现文本走 note),
  补不打桩单测。
- **H2 awaiting 谓词二义**:views「有任意 decision 即非等待」vs 问询「按 question_id
  配对」——同案二次欠定(F6 新增来源)会被重派或漏停车。修:对齐为按 question_id。

**F2 裁决侧知识义务**:(a) attributor 白名单补 `compile_precedent`(prompt :57 指令
与白名单矛盾,V6 遗传纯缺陷,一行修;loader/tool_gating 零联动——已核)——**立即**;
(b) bed/缺陷类 ask 呈报前携先例检索回执(锚 **§11.11 构件一** retrieval_receipt;
bed 问询是纯机械产物、路径上无检索步,需新增引擎侧机械检索)——**并入 S₀ 前置
裁决批**(原名「G5 前置自动裁决」,为消一名两义改此名),免与其重做返工。

**F5 投毒清算二批(机械)**:dongkl 3 卷(105877/105910/681783)隔离出 mirror+
footprint 引用复核;(登记)93 跳板机可达则审 zhaiyq 22/50 翻转原始数据。

**F7 测试要点提取义务(C 层)**:worker 编写前先陈述本案 claim 与证伪观测;机制
替代的合法性按 F6 等价判据(四条,见下)对照,差异如实声明。**删除**
compile-worker.md:57 reload-from-saved 范例(领域答案示例红线违例,100% 套用率
实证);**同批必改** DESIGN §5「机制替代(clear+reload 代真重启)须在 D 列声明」
条款(与本节相抵的旧文,评审 D13);worker md 行数账**唯一现行口径=§5.5 ⑥C**（归属行+
150 熔断；**废固定行数预算**——F7 时点记 86 行、2026-07-17 实测 206 行等历史数字
勿再引作限额）。worker md :49「等价变体可径取不问」授权行**收窄**:
禁令机制族除外(评审 T12)。

**F6 禁令机制意图路由(核心件;两前置+同批五改)**:
- **前置①=H1**;**前置②意图侧禁令词表**:destructive_commands 是英文命令正则,
  匹配不到中文意图文本(「重启」);意图侧词表(重启/断电/恢复出厂族,CJK 安全边界,
  带 provenance 与误报自证说明)为**必须新增的文法数据段**——承认数据增长,
  「零增长」宣称撤回(评审 D4)。强制点=A 层:author `_stamp_intent` 时对意图文本
  扫词表,命中即 stamp `forbidden_mechanism` 标记;worker 义务为 C 层加固非强制。
  **（地位后被 §18.13 撤退清单重分类,2026-07-17 补指针）**:该词表判「违反结构化
  红线,该退」,分两步——第一步只移 brief 盖章块、词表+emit 门保留为**安全 backstop**
  (已执行,briefs.py:242);第二步对照轮自主呈报率达标后才评估删词表+门。本段
  「必须新增」是建立时点的裁决,长期地位以 §18.13 为准。
- 路由:命中 → 强制 `compile_report_underdetermined`(claim_kind=
  `forbidden_mechanism`,**不走** compile_check_verifiability 的闭集校验;
  questions.py 加对应分支,题面携等价实现提案),先问后落代码。
- **同键采信豁免(评审 T12)**:判例键=(意图签名×forbidden_mechanism×版本族)
  命中既有裁决 → 机械采信不复问((20) 收敛律);编译期无回显,采信条件用
  「意图签名精确匹配」变体(B 片回显子串条件不适用)。
- **等价判据四条(评审 T1,取代四稿三条)**:同面清除(作用域=σ 差集∪π 支撑分量,
  非全面——全面清除即 A 层禁令)∧ 证伪观测不变 ∧ 不引入反向(导入)操作 ∧
  **对被测机制保持敏感**(删掉被测写步应改变判决;clear 形对 write 步在「模型违背
  世界」不敏感——启动通道缺口必须进差异声明,不得以「担保等价」开路,评审 T1/T6)。
- **配置面模型的效度声明(评审 T2)**:模型(运行=内存易失/write 族=导出)以
  「用户域教学 K_ought 判例+可错声明」入文法层,**非无条件事实**——版本族锚+
  设备验证状态=未验+**已登记仓内张力**:手册 HA bootup synconfig 节明文「启动时
  同步 write memory 保存的配置」(app_21 §synconfig)——HA bootup 模式除外声明
  必须随模型下发;证伪路径=授权床一次性重启实验(登记,(25) 人批高危)。
- **写保存族正解(模型条件下)**:改过程「重启→clear 验证」——配置→观测→write
  导出→clear(作用域=本案 σ 差集)→观测→断言 not_found→**制品清理步**(评审 T13:
  write 产物是制品子面残留,τ 配对恢复门落地之日无清理步的卷全族被呈报;
  「比现有卷少一个回放步」改记为「删一回放步、增一制品清理步」)。
- 面板选项序:①改过程(主推,携等价实现+**模型条件**等价论证+差异声明)
  ②用户另给等价(映射「改过程」+原文入 note——Other 自由文本兜底需补,评审 D9)
  ③如实跳过/挂起(须携模型下穷举论证;枚举基=clear 格节点∪inverse_forms∪其组合,
  评审 T18)。
- **同批五改**:P1c 提门(触发锚改 intent.json 保存族词,不再以 restore 存在为
  前提——否则 F6 正解卷 P1c 全族失效,评审 D2);两处门反馈文案(destructive 反馈
  「save→clear→restore→observe」与 pairing 收尾「assert presence」)改为机制陈述
  指向文法段(评审 D3);questions.py 长度不变量与 ask_shown 代表映射(F8c);
  视图谓词 H2。

**F8 族感知编译**:脑图组(同父叶簇)是语义单元,设计不再原子化。理论引用带帽:
纤维性等式主语是脑图一级节点(功能域粒度),移植到叶簇是**外推**;r_eff≈4.8/骨架
重合 45-51% 为解释性非判据(K R7-M7)。构件:
- (a) **兄弟上下文进 brief**(存活,改形态):内联=兄弟 autoid+title+**机制轴**
  一行式;check 全文**按引用**(manifest_path 既有)——X8 合规+压 (37) 锚定面
  (兄弟期望值不内联,防 precedent-then-assert 近亲,评审 D14/D15)。
- (b) **组级要点(重定义,评审 D22/T14)**:~~引擎侧 LLM 组提取~~(撞 §19.2 brief
  契约「不承载预消化知识选择」+新孔禁令+错误一致化)——改为**机械形态并入 (a)**:
  brief 组块携 group_path+兄弟行,F7 义务升组措辞(「先陈述组共享 claim 与本案
  变体轴」),由各 worker 在兄弟可见下自行提取;若未来确需组级一次提取,必须按
  (37)① 待证形态注入+worker 异议出口+显式修订 §19.2,另行立项。
- (c) **组一题**:needs_decision 按(组,claim_kind)折叠,代表案提问、答案扇出
  **逐案落盘** user_decision.json(emit 门按案读);修 validate_questions 长度
  不变量+ask_shown 代表映射(评审 D16)。
- (d) **兄弟碰撞门**:消费 (b) 显式变体轴,同组同轴取值=呈报(**D5 型不硬拒**——
  组变体轴非机制轴的组会假阳,评审 T14 附;硬拒姿势仅保存族闭集已证成)。
- (e) 编排边界:保持 1 案=1 worker,关联走共享只读上下文(不变)。

**F3 判例分层与防自指((45) 工程面)**:血统标注三档 `human_suite` /
`human_adjudicated`(用户 adopted/correct 背书的机生卷,评审 T9)/ `engine_verified`
(relpath 判源,语料需补相对路径字段);检索**配额保底**(结果集含≥1 人源命中,
若存在)+血统标签外显;机制 token 轴入意图索引。**未经窗口审计的写回=uncertain 级**
——含 `window_audit: unavailable` 时的新写回(否则「经审计」成新免检图章,INV-11);
uncertain 效度在语料契约加字段。**极性禁运(定界版,评审 D18/T8)**:辖区=
`compile_precedent` 相似度语料的**卷面极性**(禁跨意图照抄断言方向/期望值);
**例外**=判例键精确命中的 K_ought 裁决判例(用户裁决/adopted——(21)/A11 合法
采信面不动);与 F1 五查划界:第三源极性只作**面板对照证据**,不作断言书写来源。
三处现役表述(worker md :26-28「or a verified precedent」/precedent_tools 格式化
文案/§19.2 溯源红线)**同批改齐**,否则 worker 收自相矛盾指令。

**F1 期望可疑臂(收紧形态,评审 D21/T5/T6)**:
- (40) 处置分类学**增补第七类**(expectation_suspect,唯一出口=F1 面板;终止性
  由 (20) 同键一次担保)——满射不变量同步,勿静默扩类。
- 词表 `_DISPS` 扩一员 + **panel 併呈机械门**(submit_attribution 落
  expectation_suspect 必须携同轮 submit_ask_panel,否则活锁:author 不重编、
  merge 不复跑、无 ask 目标);裁决结果**折叠回既有词表**(确认缺陷→
  defect_candidate@99;修订期望→correct+reflow;挂起→suspended)——**不发明
  @99 新终态**(views/report_gate 双路冗余是 G5 报告门故意设计,勿协同改)。
- **五查=程序义务**(评审 T6):第五查(第三源极性对照)以「对照完成」为满足,
...
- ~~**S₀ 前置裁决 + F2(b) 为独立后续轮**(graph 流大改,§18.10 立项):s₀ 进 bed 问询
  ...
  **（2026-07-17 补标）**:S₀ 前置裁决整个方向已被 §18.12 撤销(数据+架构双重理由);
  其宿主批随之解散——**F2(b)(bed/缺陷类 ask 呈报前携先例检索回执)未落地且失去排期**
  (代码现状:retrieval_receipt 仅 questions.py panel 通道消费,bed 路径零命中),
  待重新排期或显式缓立。F2(a) 已落地(attributor 白名单含 compile_precedent)。

**F4 执行器契约入文法层(已落地,2026-07-14 批次七;句法门经审查不建)**:实核
mirror `lib/test_xlsx.py::get_parameter`——`,prompt=<value>` 是 cmd_config 的**自包含
kwarg**(框架在交互提示处自动发送 value,无需额外响应步),`,timeout=` 覆写步墙钟。
**判定**:①文法数据段 `executor_contract` 落地(语法+自包含语义,出处=get_parameter
源码+人工卷 log_backup)+ worker md 一句 C 层知识(交互命令用 `,prompt=` 自包含,
从先例检索形态)——写保存族 YES 错位的**知识面**对策;②**句法配对门不建**(充分
审查后诚实结论):交互确认是**条件性**设备行为(文件已存在才提示),"哪些命令需
prompt=" 无纯句法配对可建——与 interactive_confirm 猜测门同理(AUDIT_gate_inventory
§2.2 决定不建)。硬造会假阳(误伤不触发确认的正常命令),违「改片面只匹配当前场景」
的两向压力测试。F4 收敛为数据+知识,不落门——防止为凑构件发明伪门。

**ask 上游化原则(设计条款)**:面板必须从 (35) 判决三链的**最上游断点**生成——
意图链断裂在编写期问(F6),不许在过程链/对象链症状(床态)上代偿提问。
run18-21 ask 风暴即此原则缺位实证。

**落地序(五稿定版)**:F2(a) → F5 → F7(含 §5 旧条款修订) → **H1+H2+F6 前置②**
→ F6+F8c(+P1c 提门+门文案) → F8a(含 b 机械形态)+F8d → F3(含定界极性禁运,
三处同改) → F1+~~S₀ 前置裁决+F2(b)~~ → F4。
**（2026-07-17 补标）**:序中「S₀ 前置裁决」已被 §18.12 撤销;F2(b) 随宿主批解散、
未落地待重排期(见上 F1 段补标)。其余各步落地状态以各构件段内标注为准。
...
### 18.14 对象链合取的系统性回补（aha,2026-07-15;两审计独立收敛)

**背景**:run23 后并行两审计——代码设计审计(5 路并行+逐条对抗验证)与 langfuse
运行时审计(41 真实 worker trace)——从**引擎判据侧**和 **worker 推理侧**独立收敛到
同一根因。

**aha:实现层系统性地在机械化理论时丢掉「对象链」合取,退化成过程/构造近似。**
理论(K (35) 三链=意图∧对象∧过程;infra T1=共享∧未复位∧**读脏态**;S 等价判据四条
含「对被测机制敏感」)**写全了对象链要求**,但每次落地反复漏掉它:

| 实现处 | 理论要求的对象链项 | 实现丢成的过程近似 | 假象 |
|---|---|---|---|
| s₀ 谓词(I6) | T1「读脏态」(床对象状态) | 命令文本/IP 交集(过程共享) | persist/L2L3 假阳 |
| 可达门 | 对象实际存在(接口在不在) | 静态拓扑集(过程可达) | VLAN 框死 |
| worker 断言 | K 对象链(读缺陷显现的层) | 过程链(命令能不能跑) | 83% vs 22% |
| 等价判据 | 「删写步应改判决」(对象敏感) | 「对写命令敏感」(过程) | 668015 中途 wrong-object |
| 题面渲染 | 人话对象 | 机读过程取证串 | 668059 机器味 |

**关键界定(两审计核验)**:**理论全对、不动理论文档**——s₀ BLOCKER 从「理论问题」
改判「实现保真缺口」(T1 三合取正确,I6 机械化时丢了脏态合取);两个「数据缺口」
改判设计(数据早在 network_topology.json,缺代码接线)。**修法=让实现忠实已写对的
理论,系统性把每处丢掉的对象链合取补回**,非推翻重来。

**统一解释**:窗口失真(读错窗口对象)、s₀ 假阳(过程共享当对象脏态)、可达假阳
(过程可达当对象存在)、禁令词表(过程字面当对象语义)、668015(过程写敏感当对象
敏感)——同一病。K §2.9.1 早预言「delta 聚集在 intent→object 接缝」。

**修复单元(按 aha 组织)**:
- **对象链回补**:S1(s₀ 补脏态·排固定基础设施 IP,BLOCKER)/ S2(可达认动态接口)/
  L1(等价判据补对象敏感)/ L2(同族对象锚定共享 claim)/ S3+D2(题面人话对象);
- **活锁/缺陷**:D1(非 mech 臂经 ask_decision 活锁,MAJOR-live)/ D3(forbidden 残留)/
  D4(TUI 面板 dismiss);
- **文档对账**:S4(§18.7 锚纪律/§18.11/18.13 header/schema 展平);
...
> **⚠️ 本节单元 A-D 已被 `docs/DESIGN_dongkl_finalization.md` 定稿取代(2026-07-15,经真实 Langfuse 数据
> 四报告取证 + 用户逐条裁决)；单元 E（机械化 benchmark/naive 基线/消融的 eval 基建）未进入定稿——
> 非被取代亦非被裁决,仍为独立立项待办(2026-07-17 审计修正:原「A-E」措辞对 E 落空,team4_design_audit P0-1;
> E 是理论 C1「DS-2 准入门」与 eval-first 纪律的工程载体,建/缓立/不建待用户裁决)。**单元 E 定谳（②B，用户 2026-07-19：缓立）**：E **缓立、不立门**；**转正条件**（二选一满足即重启立项）=① DS-2 意图保真集种子 ≥30 对（够金标准可 benchmark），或 ② 真实评审器候选出现（有可对标 oracle、非 naive 基线空转）。此为**显式缓立、非遗漏**——补反向指针纪律「逐单元交代去向（含缓立/取消）」要求的那一句（治 team4_design_audit P0-1「A-E 措辞对 E 落空」）。定稿把本节两处判断纠回**:
> ① oracle 的「集合相等/比例不可构造→需单元 D」**是错的**——membership 用 abs_found 合取、分布用大样本+
> 区间正则均可构造(domain_grammar 既载),**不需要 D**;② 机械后校验(A/B)改为**「摆事实+prompt 自查」,
> 不加机械门替判**(用户红线)。写回护栏改为**像记忆一样如实标+用前先核**;broken 采纳 pyATS 七码
> (仅协议级硬码)为完整交付;command_existence 不修(worker 自纠)。下方原探索性文本保留作演进记录,
> A-D 的实现以定稿文档为准。
...
### 18.16 folding 门变体 A：先问后落门的折叠凭证抗伪（2026-07-18 定案，Py-Eng+Theory+Design+TUI-Eng 四方共署；F-Doc-1 回填）

**实弹（599838）**：gather 批末折叠组（同 group_path+意图签名的欠定案聚成一题，代表案提问、答案扇出全组）——**非代表成员的先问后落门假拒**。机理：先问后落门（`verifiability_tool.py:405-426` `compile_user_decision`）落盘前扫 `ask_user_answers.jsonl` 查是否有含该案尾6/全 aid 的行；但折叠面板 `build_questions({代表 aid})` 只含代表案、`interrupt` 只落**代表**问题文本 → **成员尾6 从未入 Q&A 日志**（grep：代表×1 行、成员×0 行）→ 门查不到 → 返 error → `_land` 落盘失败 → 案回 needs_decision → 下轮重问。**门与 facts 两侧折叠口径不一致**（facts 的 `ask_shown` 逐成员落、门只见代表案）。机读坐实排除 TUI payload 污染，归引擎侧；非本次引入（门 2026-07-05、折叠既有）。

**门的抗伪设计意图（不可弱化的红线）**：先问后落门防 orchestrator 在 ask_user 前替用户拍板（曾替 8 欠定案自证入账）——**只有「真实问答路径落的独立凭证」是 A 层**，门的抗伪核心=**独立于引擎/orchestrator 的自我断言**（凭证路不挂编辑路，`[[gates-on-credential-path-not-edit-path]]`）。三候选评估中，读引擎 facts（folded_into）的候选③被否为**准则违反**（门读被审计者账=可伪造绕门，违审计器权威公式18）——非选型偏好。

**变体 A（定案，凭证路折叠成员锚）**：
- **凭证形态**：代表案的**真实应答记录**（`ask_user/__init__.py:244` 唯一写点，凭证路）带 `folded_members:[全 aid]` 专用字段——引擎把成员清单塞进代表题 dict → `_bridge→_panel`（engine_tool.py）→ `ask_user.func` → :247 落盘。**专用字段不复用可截断的 `question[:500]` 题文后缀**（截断正是 599838 根因）。凭证条数=真实应答批数（⌈N/4⌉），不膨胀。
- **门匹配结构化**：门（verifiability_tool:415）从子串 `aid[-6:] in _line` 改**集合成员判定** `aid in rec["folded_members"]`（全 aid 精确）——顺带根治裸数字尾6子串误撞（`[[transient-error-bare-digit-marker]]` 同型）。写(:247)读(:415)同步改；旧记录（无 folded_members 键）退全 aid 子串兜底（兼容）；损坏行 json 解析失败 continue 跳过、fail-closed（无有效凭证则拒，不误放行）。
- **两路分立（(18b) 辖区判据；正本见 `THEORY_k_state_machine.md §5.5.6`）**：facts 的 `ask_shown`/`folded_into` **留作审计/回放（INV-7 辖业务态派生）、不作门凭证**；门凭证走独立应答路径（凭证路，独立于被检查方）。**判据摘要**（folding 场景投影，全式见正本）：业务态（counts/verdict/deliverable）归 facts 单源防显示漂移、防伪凭证归独立凭证路防自证；抗伪不来自「凭证在流外」、来自**写入点相对外部信号（interrupt resume）的时序位置**——post-signal 才抗伪、凭证甚至可进 facts；精确判据＝门不能读被检查方在凭证信号之前能自达写入的账（旧「门不能读引擎写的账」过粗）。变体 A 选独立凭证路（`ask_user` 工具 `event.wait→write` **结构锁定** post-signal）是「post-signal 下独立凭证路 > 塞 facts 单源」推论的直接结果。**全式（角色二分/时序精确判据/推论 A 独立路优先·B F 豁免二分/守门族 (48)·(16)·(18b) 三口封闭）见 THEORY §5.5.6 正本**——设计侧不重复，互为投影。

**抗伪不变量（三源恒等，可机械守）**：`凭证.folded_members == 代表题面披露的成员集（nodes.py:653-655 tails）== 门放行集`——三者同源 `fold[rep]`、恒等。抗伪边界=「折叠披露与凭证一致」（机械守，非无界信任）。

**知情同意串联链（Theory 修正，`team4_fold_gate_eval.md §6.4`）**：三源恒等只守「凭证==组装披露」（凭证层）；完整知情同意还需「组装披露==用户实际看到」（展示层：`ask_user/__init__.py:247` 题文 `[:500]` 截断修）——否则用户因截断只看到部分成员、门却按全集放行=**知情同意假成立**。两层**串联依赖、缺一环链断**：展示层截断修是凭证层守门恒等真成立的**前置**（非并列可选）；守门测试注释须点明「本测试只保证组装侧恒等，用户侧完整送达依赖 :247 截断修复」。

**批级并集安全性**（Design 验证）：一次 ask_user 调用落一行、含全批成员并集；门在 `_land` 时查、只对**用户答了的题**的成员落 decision，且一 aid 只属一 rep（`_rep_of.setdefault`）——故「aid 在批级并集」⟺「aid 的唯一题被答」，**无跨题误放行**（批级并集只是记录形式，门锚定实际答的题）。

**守门测试（宪法级，验非假门；§6.5 的 T1–T4 + F-Py-1 健壮性一条）**：①折叠正确性——凭证 folded_members 恰等于题文披露成员（防引擎偷塞用户没看到的案，T1）；②折叠成员集合判定过门——599838 反向（真答成员放行，T2）；③截断免疫——大折叠组全 aid 清单不被 `[:500]` 截断丢（599838 根因锚，凭证层，T3）；④展示送达——题面披露成员经 :247 修复后完整送达用户侧（串联链下半环，展示层，T4）；⑤损坏行不污染有效行（fail-closed 只对「真无有效凭证」、不误伤「有有效凭证但夹损坏行」的案）。实施域纯 Py-Eng（nodes.py+ask_user+verifiability_tool+测试），无 TUI 渲染改动。

## 18.17 数据集承接（④B，用户 2026-07-19 裁；DS-1/2/3 消费挂钩，正本定义见 THEORY §7）
<!-- 原误标 ## 18.16（与上行 ### 18.16 folding 门变体 A 撞号）；编号只增不改 → 升为 18.17 -->

理论 §7 的金标准数据集（`knowledge/data/theory_eval/`，JSONL、git tracked）落地后**必须挂消费点**，否则建了没人用（同 C5 footprint「供给有消费无」的治理缺口教训）。三条承接契约：

- **DS-1 归因金标准集 → 消费挂钩：改归因链必跑 DS-1 回归**。任何触 attributor 判层 / disposition / 归因逻辑的改动，上卷前必须对 DS-1（含三条 wrong_attribution 反例）跑回归——防归因改动静默劣化（那三条是既有强处置误判的钉子，THEORY §7）。
- **DS-2 意图保真集 → 评审器候选必过准入考场**。任何"自动评审器 / oracle 候选"（含 §18.15 单元 E 转正的候选）上线前，必须先过 DS-2 准入考场（种子 ≥30 对，即 §18.15 E 转正条件①）——**LLM 判断永不单独充当对自身产物的质量门**（grade 之死红线，THEORY §665/DS-2）。
- **DS-3 → 建设前置照理论 §7**。DS-3 的建设按 THEORY §7 规格前置（数据形态 / 采集目标 / 落点）——**正本单源在 §7，本节不重述**，只挂 DESIGN 侧承接指针（避双写漂移，同零双写纪律）。

**承接纪律（一句）**：数据集不是"建完存档"就完事——**每个 DS 必须有一个"改了什么就必须重跑它"的消费点**（DS-1↔归因链回归 / DS-2↔评审器准入 / DS-3↔§7 前置）。无消费点的数据集=死数据（同 C5：判例层供给有、消费无=治理缺口）。
...
(活语法)。**溯源红线**:断言期望值**极性(found/not_found)与目标**溯源手册/意图
(及同键用户裁决判例);先例只供**配置形态**,不供断言方向——异意图先例可断言相反极性,
照抄=假验证(precedent-then-assert,observe-then-assert 的姊妹,2026-07-14 F3 定界)。
照抄设备当前回显同禁(全仓铁律)。
