...
- 预算：worker md ≤60 行、attributor ≤50 行、SKILL.md ≤80 行；每条存留规则在
  ...
  **豁免登记(2026-07-14 F7)**:worker md 现 99 行——F7 要点先行节(+13,§18.11
  写保存族 σ 链断裂的直接对策)按「三选一存在性检验」通过(不写=四案×两代引擎
  100% 重蹈已实证);下一次减法轮的置换候选=检索序细节(footprint 工具 docstring
  已载)。攻击面账:超预算是显性登记的债,非静默漂移。
...
条款(与本节相抵的旧文,评审 D13);worker md 已超 §5.5 60 行预算(现 86 行)——
等量置换或在 §5.5 记豁免。worker md :49「等价变体可径取不问」授权行**收窄**:
禁令机制族除外(评审 T12)。
