...
的 prompt 写明四查含 `compile_precedent` 同意图对照,但 tools 白名单从未包含该工具
...
条款(与本节相抵的旧文,评审 D13);worker md 已超 §5.5 60 行预算(F7 时点记
86 行;**行数账以 §5.5 豁免登记段为唯一现行数字**——2026-07-17 实测已 206 行,
勿再引本处历史数字)。worker md :49「等价变体可径取不问」授权行**收窄**:
禁令机制族除外(评审 T12)。
...
## 18.16 数据集承接（④B，用户 2026-07-19 裁；DS-1/2/3 消费挂钩，正本定义见 THEORY §7）
