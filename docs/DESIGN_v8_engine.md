...
| 辖区判据（公式18b，2026-07-18 理论新增·Theory 起草） | 数据归 INV-7「业务态以事实流为准」还是「门挂独立凭证路」取决于**角色**：业务态（counts/verdict/deliverable）须 facts 单源防显示漂移；防伪凭证须独立于被检查方防自证。**精确判据＝门不能读被检查方在凭证信号（interrupt resume）之前能自达写入的账**（写点 post-signal 即抗伪；凭证可进 facts——抗伪不来自"在流外"、来自写点时序位置）。folding 门变体 A 首个实证，详见 §18.16 | 旧「门不能读引擎写的账」过粗——引擎 resume 之后才能写的合法凭证被误禁 |
...
- **两路分立（(18b) 辖区判据，Theory 定稿；source: `team4_fold_gate_eval.md §6.1/§6.7`）**：facts 的 `ask_shown`/`folded_into` **留作审计/回放（INV-7 辖业务态派生）、不作门凭证**；门凭证走独立应答路径（凭证路）。**辖区判据全式**：数据归 INV-7「业务态以事实流为准」还是「门挂独立凭证路」取决于**角色**——业务态（counts/verdict/deliverable）须 facts 单源防显示漂移；防伪凭证须独立于被检查方防自证。二者不矛盾，**凭证甚至可进 facts**——抗伪不来自「凭证在流外」、来自**写入点相对外部凭证信号的时序位置**：写在外部信号（interrupt resume）之前=被检查方可自达=不抗伪；之后=需真实外部信号才可达=抗伪。**精确判据＝门不能读被检查方在凭证信号之前能自达写入的账**（旧「门不能读引擎写的账」过粗——引擎 resume 之后才能写的凭证合法）。变体 A 选独立凭证路（`ask_user` 工具 `event.wait→write` **结构锁定** post-signal），而非塞 facts（须守「写点必 post-signal」隐形不变量、无编译期守护、谁挪到 signal 前即破）——**A 更优是辖区判据的直接推论（§6.7），非工程妥协**。
