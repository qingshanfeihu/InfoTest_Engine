...
- **G5 前置自动裁决(设计定形,立项——graph 流改动单独轮)**:s₀ 判定进 bed 问询前,
...
### 18.11 是-应对称与判例分层（修复方案候选,2026-07-14;全节**设计中**,待用户拍板）

理论位:K §2.6.6 对称怀疑推论 + K §2.9.4 (45)(45b) 判例源权威与防自指公理 +
矛盾即问边(K §8)。以下五个构件为方案候选,依 §18.7 完成度纪律,落地前引用必须
带「设计中」:

- **F1 期望可疑臂(设计中)**:归因词表增 `expectation_suspect` 处置——判据:设备
  行为与脑图期望冲突 ∧ 案自身写法排除(经窗口审计的干净判定)∧ 第三源对照完成。
  缺陷候选四查升五查:第五查=**第三源极性对照**(人源判例/手册/缺陷库,机制同构
  优先)。呈报走 §11.11 既有裁决面板,新增「脑图 vs 设备」入口边:题面双侧引文=
  设备原始流原文 + 第三源记载原文(verbatim 子串门既有机制复用),选项=确认缺陷
  候选/修订期望(人源专属)/挂起待研发。
- **F2 裁决侧知识义务(设计中;含一处纯缺陷修复)**:attributor 白名单补
  `compile_precedent`(prompt 指令与白名单矛盾的一行修);「山穷水尽」扩展到知识面
  ——bed/缺陷类 ask 呈报前须携**先例检索回执**(§11.7 retrieval_receipt 机制复用,
  空手问 schema 级不可能的既有构造直接延伸)。
- **F3 判例分层与防自指((45) 工程面,设计中)**:先例语料按血统标注
  (`human_suite` / `engine_verified`);检索排名人源优先或配额保底;机生判例
  仅在其判定经窗口审计后按 verified 采信,历史未审计写回降 uncertain 级;
  意图索引增**机制 token 轴**(write/config 族、交互确认形态——cfg_tokens 既有,
  补机制维度),治 log_backup 型"机制同构、词面遥远"检索不中。
- **F4 执行器契约入文法层(设计中)**:`,prompt=` / 确认词步 / `,timeout=` 是框架
  执行器**语法**(mirror test_xlsx.py 可推导+人工卷 40 例实证),提为 domain_grammar
  新段;emit 侧可建**句法配对门**:带 prompt= 的命令后必须跟确认词步——注意与
  §18.10 先前「interactive_confirm 门决定不建」的相容性:不建的是"设备会不会提示"
  的**猜测门**(条件性行为),可建的是"框架语法自洽"的**配对门**(纯句法,零猜测)。
- **F5 投毒清算第二批(机械执行项)**:dongkl 3 卷隔离(同 yzg 处置)+ 其 footprint
  引用复核;(登记)93 跳板机可达则审 zhaiyq 22/50 翻转原始数据。

落地序建议:F2(一行修+回执复用,最小)→ F5(机械清算)→ F3(检索面,防再投毒)
→ F1(词表+面板边,与 G5 前置自动裁决同批)→ F4(文法提取+句法门)。

**质疑性探索修订(2026-07-14 二稿;用户两向压力测试驱动——激进向=盲抄先例/真重启,
片面向=场景正则+无正解 ask)**:写保存族的真断点在 **σ 链(意图→步骤)**:测试要点=
「保存后重启不残留」,期望无错,步骤把重启换成语义相反的 config 回放——不存在真
是-应冲突,F1 对本案类**不适用**(F1 只服务真冲突)。引导源钉到行:
`compile-worker.md:57` 范例直给「reload-from-saved instead of a physical reboot」
(示例红线违例的最强实证:四案×两代引擎×每轮 100% 套用),且替代义务降为 desc 声明
——声明无人消费,十行外的欠定上报路径历史零触发。据此增补/修订:

- **F6 禁令机制意图路由(新,治本案类;设计中)**:意图文本命中禁令机制族(文法闭集,
  destructive_commands 数据既有:重启/恢复出厂/断电族)→ **强制**
  `compile_report_underdetermined`(claim_kind=forbidden_mechanism),先问后落代码;
  题面=意图原文+禁令依据+替代候选**及其语义差异声明**(等价性由证伪观测是否不变判定)。
  655248(HA FIP 床不可实现→欠定呈报)是本路由产出正解 ask 的活样本。零场景正则
  增长:触发闭集=既有禁令数据的意图侧投影。
- **F7 测试要点提取义务(新,C 层;设计中)**:worker 编写前先陈述本案 claim 与
  **证伪观测**,机制替代的合法性以「证伪观测不变」为判据;同时**删除**
  compile-worker.md:57 的 reload-from-saved 范例(领域答案示例,红线违例)。
- **F3 增补——极性禁运**:先例(无论血统)只供**形态/机制**知识;断言极性与期望值
  只准溯源脑图+手册(observe-then-assert 红线的姊妹:precedent-then-assert 同禁)。
  防激进向失败模式:log_backup 的 found 极性被抄进不残留类意图=更自信地错。
- **F1 降位**:写保存族下批走 F6 面板,非缺陷/期望裁决。
- **F6 四稿——山穷水尽的方向性(2026-07-14 用户三次裁决;S §0.3 持久面细分+等价
  判据)**:配置面模型(运行=内存易失/write 族=导出/无启动面)不变;**「山穷水尽」
  的目标是有能力完成设计验证就实现**——面板的首要义务是给出模型下推导的**等价
  实现**,不是罗列体面的投降。两个对偶失效模式同日双实证:二稿**发明伪替代**
  (查保存载荷——测反,未过等价判据);三稿**漏掉真替代**(跳过/挂起/换床——
  未在模型下穷举同面实现)。**等价判据三条**(S §0.3 推论①):同面清除(实现同一
  前件)∧ 证伪观测不变 ∧ 不引入反向(导入)操作。写保存族的正解:**改过程
  「重启→clear 验证」**(clear 与重启同为运行面清除,无启动面模型担保等价;
  正确编译=配置→观测→write 导出→clear→观测→断言 not_found,比现有卷**少一个**
  回放步)。F6 面板选项序:①改过程(主推,携带等价实现+等价性论证)②用户另给
  等价实现 ③如实跳过/挂起(仅当模型下穷举论证「无有效替代」——缺论证的投降与
  缺判据的发明同罪)。「listener 是否应入保存域」规格问询维持撤回。配置面模型
  提为文法层数据段(手册「备份到…文件」措辞出处+用户域教学 K_ought 记载),
  worker 编写、F7 要点提取、F6 等价推导三处共同消费。
- **ask 上游化原则(设计条款)**:面板必须从三链分解((35))的**最上游断点**生成——
  σ 链断裂问 σ 链问题(编写期),不许在 V 链症状(床态)上代偿提问。run18-21 的
  ask 风暴(问一堆无一正解)即此原则缺位的实证。

- **F8 族感知编译(新,2026-07-14;设计中)**:现行设计把脑图原子化——1 案=1 worker,
  brief 只带本案意图+组路径字符串,**兄弟案意图零可见**。但脑图组(同父叶簇)是语义
  单元:配置保存组四案共享同一 claim(「导出不产生持久性,重启即失」),按保存变体
  实例化——理论早有此结构(K §2.11 意图空间低秩 r_eff≈4.8、族内骨架重合 45-51%;
  S §0.3 域纤维性:脑图组=功能域=同一分区单元),设计从未消费。原子化成本实弹:
  四个 worker 各自独立发明同一个错误替代(重启→回放),错四遍且互不知情;668030
  漂移撞 668000 无人能见;配置面模型的破案靠人工**整列阅读**——列级视角应是设计
  内动作,不是事故。构件:
  (a) **兄弟上下文进 brief**:同组 siblings 的 title+check 原文(只读;manifest
      group_path 现有,零新解析);
  (b) **F7 升组粒度**:测试要点提取在**组级**做一次(组 claim+证伪观测),组内
      brief 共享,变体轴显式声明(「本案=该 claim 的 write file 变体」);
  (c) **F6/面板组路由**:禁令机制命中组 claim → **组一题**(共因合题机构的编写期
      前移,run15「11 案恢复=一题」同型复用);
  (d) **兄弟碰撞门**:同组两案机制轴取值相同=撞题(668030 漂移≡668000 型)——
      manifest 组+卷面机制 token 机械比对,P1c 的组级推广;
  (e) **编排边界(防激进向)**:保持 1 案=1 worker(自由理解隔离不动)——关联性走
      **共享只读上下文**,不走合并编写(组 worker=错误全组一致化+爆炸半径放大;
      紧耦合族可选组 worker,默认不)。

落地序更新:F2 → F5 → **F7+F8b(组级要点义务,纯 prompt+组聚合)** → **F6+F8c
(欠定路由组一题)** → F8a/F8d(brief 兄弟上下文+碰撞门)→ F3(含极性禁运)→
F1(真冲突面板,与 G5 同批)→ F4。
