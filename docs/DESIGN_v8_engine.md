...
- **F6 三稿——配置面模型修正(2026-07-14 用户域教学;S §0.3 持久面细分)**:运行配置
  内存易失,write 族全部是**导出**(制品面),无一创建开机持久性;「保存后重启不残留」
  的唯一证伪观测=重启,**不存在保持证伪观测不变的替代编译**——F6 二稿里「检查保存
  载荷内容」类替代候选**作废**(制品含全量配置是导出的正常语义,查载荷=又一道测反
  的题)。F6 面板对此类意图的选项收敛为诚实三件:①如实跳过/挂起(本床不可验证,
  655248 同型)②换可重启床 ③申请受控重启窗口(床权决策)。「listener 是否应入
  保存域」规格问询**撤回**(伪问题:导出本就该全量)。替代候选的合法性评估一律
  以配置面模型为基——**「无有效替代」是合法且常见的结论**,面板不得为凑选项
  发明伪替代。配置面模型(运行=内存/write=导出/重启=释放)提为文法层数据段
  (手册「备份到…文件」措辞为出处,用户域教学为 K_ought 记载),worker 编写
  与 F7 要点提取共同消费。
