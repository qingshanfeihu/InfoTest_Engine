...
| S8 | 多写者边界（理论修正回写项） | —— | S 代数「配置面在算子写之间静止」的「算子」限定为**本案算子**；共享床他案写的翻转归 infra T1，不进时间轴辖区也不作其反例——待理论写入批回写 S 文档 |
...
3. **I3 mirror 同步锚 + S1 类型图解析**（同为「信任根+一次性离线解析」族：hash
   对账与签名解析同批做，S6 命令集、S7 时变签名表同管道捎带产出）；
4. **S6 命令存在性呈报门**（数据就绪即上，D5 形态）；
5. **序② diagnose+机械前筛**（X1/X2/X3/X6/X10 兜底；输入按 S2 升级为上闭包配对，
   类型图未熟先用 I6 近似）；
6. **I4 quarantine 态**；
