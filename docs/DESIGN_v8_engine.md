...
- **S₀ 前置裁决(原名 G5 前置自动裁决,2026-07-14 改名消与 G5 报告重算门的一名
  两义;设计定形,立项——graph 流改动单独轮)**:s₀ 判定进 bed 问询前,
  引擎必须先自耗尽裁决手段:①受害案**隔离复跑**一次(单案子卷;隔离过=顺序依赖坐实,
  隔离仍挂=污染假设被反驳→深归因/重编,不问)②占用者**只读探针**(show 对应对象族,
  有实体=佐证,无实体=削弱)。山穷水尽(两项都指向污染)才问,且题面只剩真正的用户
  权决策:如实降级 / 床权治理后挂起。「床已处理,复跑验证」作为用户选项**删除**——
  复跑验证是引擎的活(2026-07-14 用户裁决:"2床已处理不应该你自己处理复跑验证吗")。
