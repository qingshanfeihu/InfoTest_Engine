...
- 预算：worker md ≤60 行、attributor ≤50 行、SKILL.md ≤80 行；每条存留规则在
  ...
  **豁免登记(2026-07-17 实测刷新;历史登记 99/86 行均已过时)**:实际 worker md
  **206 行**、attributor md **167 行**、SKILL.md 52 行(达标)——worker 超预算 3.4×、
  attributor 超 3.3× 且此前**零登记**。原「超预算是显性登记的债,非静默漂移」承诺
  在 07-14 至 07-17 间失守(F6/F7/F8/§18.13/§18.14 多轮 prompt 增补未回填本账):
  这是登记纪律失效的实证,非豁免依据。**债务处置待裁决**:①按本节纪律做减法轮
  (逐条过三选一存在性检验)回到预算内;②按 R3「花钱买质量」授权修订预算数字;
  ③废除行数预算、改为 theory-map.md 归属覆盖率约束。三选一归用户,选定前本账
  为唯一现行数字(引用行数以本段为准,勿再引 99/86)。
...
> E 是理论 C1「DS-2 准入门」与 eval-first 纪律的工程载体,建/缓立/不建待用户裁决)。定稿把本节两处判断纠回**:
