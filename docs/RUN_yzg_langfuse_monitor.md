...
## 第三回合：V8 引擎真机验收（@93，2026-07-10）

**终局 `delivered_with_labels`：23/26 可交付 + 3 案如实分层标注**（668015 contradicted / 668030 failed / 668044 failed_terminal-用户裁决接受单跑）。与 V6 同批同床的「名义 26/26(实吞 3 案)」构成最终对照——**同样的设备真相(恰好同三案),V6 吞掉+毒写回,V8 如实呈现+自动回滚(rollback×5)**。全程 cmux+Langfuse 监控,¥44,~70 分钟。

**核心公理全部真机兑现**：
- 床态体检门:版本锚同族放行(585vs568=(10,5) match,不误拦)+异物三项探出→床权 ask 面板真机首弹(INV-9);
- 通道①排序:四保存案(write memory/file/all/net)被文法数据精确识别排卷尾;
- 全射对账:83 条裁决全入流,零吞;终验反证已交付态(668015 R1 过/终验挂)→ **自动 rollback 先例库**——#10 病灶的解在真机兑现;
- 归因定向:668044 判 rerun_isolated(卷面没问题,隔离复跑)——矛盾首环不盲改卷;668015 找到机理(config file 装载海量回显淹断言);
- 三条 ask 边全部真机走通:床态体检 → (本批零欠定) → **矛盾 ask**(单跑过/整卷第2次失败,题面完整带矛盾史) 。

**验收发现 7 项(全部当场修复或记录)**:#1 bed 探针 body 过滤漏元数据行(修);#2 final_verify_failed/verdict_unconsumed 未入信号闭集(修);#3 该信号对无 pass 前史的 fail 过宽(修:收窄到翻转已交付态);#4 矛盾谓词漏 delivery-pass→delivery-fail 翻转形态(修);#5 写回风暴(幂等无害,效率观察);#6 attribution 幂等键 (aid,round) 同轮二次归因静默去重(修:按 run_id);#7 ask_contradiction 出边缺归因回环目标+非交互空转风险(修:补 attribute 边+防空转护条)。

**V8↔TUI 兼容(用户点题)**:footer 计数键/相位词双错位确诊——V8 emit_tick 翻译到 V6 定稿的九态显示契约(前端零改动),TUI 相位表补 V8 六节点,reducer 终态判定补 closing;max 思考尾标在 V8 下实测正常。

**保存族互扰的新证据**:668044 同卷面第一次子集过、第二次子集挂(自己留在 tftp 的文件改变重跑行为——持久态自扰);668030 前两次终验过第三次挂——非确定互扰,矛盾 ask 与床账是正确的长期解。
