...
> ⚠️ **历史存档（2026-07-17 归档,team4 docs 整编）**：v3 修订计划,被 V4 取代。事实存档不删,现状勿引本文。
...
| `fail_attribution.py` + `compile_attribute` | **保留** | ③，零额外 fork |
...
- 流程对齐 V2：版本校验 → prep → `compile_fanout(ist_draft_v3)` 逐 case 并发（**不先编骨架**）→ `compile_fanout(ist_grade_v3)` → 判定/重做 → 合并。
