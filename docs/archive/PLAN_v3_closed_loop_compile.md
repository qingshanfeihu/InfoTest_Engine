...
> ⚠️ **历史存档（2026-07-17 归档,team4 docs 整编）**：v3 计划,V3R 自述其实测负收益并修订,被后续演进取代。事实存档不删,现状勿引本文。
...
- `compile_fanout` 的并发从"调用方传死值"改为**编排器按族数+端点配额算**：`min(_MAX_FANOUT, max(4, 待编译数))`，env `IST_FANOUT_CONCURRENCY` 仍可硬覆盖。
...
- `compile_emit` 增 `provenance_json=""` 入参（默认空＝V2 行为不变）：传入则把 IR 落 `<out>/case.provenance.json` 旁挂 xlsx（xlsx 本身不变，框架照吃）。
