...
> ⚠️ **历史存档（2026-07-17 归档,team4 docs 整编）**：draft/grade 编排(v4 时代),grade 2026-07-07 删。事实存档不删,现状勿引本文。
...
全部在 main agent 内部、用平台自有的 fork skill + `invoke_skill` 编排完成，不依赖外层脚本。
...
| 生成 | `ist_compile_draft` / `ist-compile-draft`（fork） | 核查前置→检索先例→`compile_emit` 生成草稿。不上机、不自评 |
| 评估 | `ist_compile_grade` / `ist-compile-grade`（fork） | `compile_score` 判断断言是否覆盖目标行为 + 给重做意见。**断言质量审批，device verdict 可选输入，不依赖上机**。不生成、不上机 |
| 上机（独立环节，非编译链） | `ist_compile_run` / `ist-compile-run`（fork，由 `ist_verify` 调用） | `dev_run_case` 上机执行 + 采集框架真实裁决（ground truth）。不修改、不评估 |
...
判定:
  ...
  连续 N 轮(建议3)仍 CUT → escalate-when-stuck / ask_user 上报
合并(compile_emit_merged) → 脑图级 excel 落地
```

### 上机验证流程（独立，走 ist_verify，产出 excel 之后）

```
ist_verify 对成品 excel → dev_run_batch 串行上机 → 采集每 case 框架真实裁决
...
- **工具注册**：`skills/loader.py` 的 `_TOOL_REGISTRY` 注册了 `compile_emit`/`dev_run_case`/`compile_score`/`compile_precedent`/`dev_probe`，fork 子 agent 才能取用。
- **置信判分** `case_compiler/confidence_f.py`：LLM 依据证据（需求 + 配置上下文 + 同类先例 + 手册行为）判分，非硬编码规则；缺判分模型时 abstain，不以硬规则猜分。`compile_score` 返回结构化 JSON（overall / decision / 逐 check_point score+reasons）。
- **检索与评估工具** `tools/device/precedent_tools.py`：`compile_precedent`（按配置结构相似度检索已验证先例，纯结构距离、无 embedding）、`compile_score`（判分入口）。
