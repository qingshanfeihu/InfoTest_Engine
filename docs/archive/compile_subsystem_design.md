...
> ⚠️ **历史存档（2026-07-17 归档,team4 docs 整编）**：v3/v4 时代子系统设计,编排层被 V6 取代、V6 又被 V8 取代(二级过时)。事实存档不删,现状勿引本文。
...
4. **期望值按可知性三分诊(条件于作者写的 config;2026-06-23 精化,详见 §8.9)**:**V_K 配置可推导值**(池 IP/超时/删后状态/rr 按序命中)→ 编译期算**常量**,不留空;**V_R 观测间关系**(会话保持/亲和性/同-异)→ H 列**捕获 + `found`/`not_found` 比较**,不留空也不编值;**V_D 设备生成的不透明单值**(auto-gen 名/PID/时间戳)→ 才写 `<RUNTIME>` 占位,由 `runtime_fill` 上机真实输出回填(结构性幂等、锁死不靠自律)。**`<RUNTIME>` 只对 V_D 正确**——对 V_R 是错的(占位只能填一个值、表达不了关系)、对 V_K 是多余的。曾把作者已知的 config 值误判为运行时不可知,凭空造出"xlsx 天花板",已推翻。
...
7. **确定性事实不耦合非确定检索**:env_facts(★ 可达 IP 等确定性事实)必须独立注入、永远可用,绝不捆进 precedent 检索——检索一空(新意图/低相似度被跳过),确定性事实跟着丢,draft 失锚瞎编 IP。
8. **确定性兜底 > 模型自律**:子 agent(尤其 haiku 档)懂概念但常放错细节(寄存器引用放 G 列、选非 ★ IP);健壮性靠确定性层(emit 归一化把 G 里的寄存器名移 H、env_facts 永注、可达闸 dig 目标白名单)兜底,不靠提示词指望模型每次放对。
9. **生成与评判共享范式**(扩展原则 5:隔离 ≠ 认知割裂):draft 产什么断言形态,grade 必须认什么形态——draft 产「捕获+比较」关系断言,grade 绝不能因"G 空 / 无字面期望值"把它当弱断言 CUT,否则正确产物被误毙、范式白做。
...
> ⚠ **本节(8.6–8.8)的"xlsx 表达力天花板"结论已被 §8.9 全面推翻(2026-06-23)。** 经源码核实 + gold 对照 + 对抗执行 + 单条端到端验证:会话保持(含多 IP、配置优先级)在**现成 xlsx 里可表达,零新函数**。下面保留原文仅作纠错链的历史记录,**结论以 §8.9 为准**。
...
# 8.9 重大校正(2026-06-23):"xlsx 天花板"被推翻 + 重校准模型 + 最小落地

> 本节正式校正 §8.6–8.8。经**源码核实 + gold 对照 + 对抗执行 + 单条端到端验证**,"xlsx 只支持精确值 oracle / 会话保持是 DSL 天花板 / 需 .py 或新原语"的结论**全部推翻**。**会话保持(含多 IP、配置优先级)在现成 xlsx 里可表达,零新函数。**

## 8.9.1 校正后的事实(Cap(xlsx) 的真实能力)

- 框架 xlsx 不是无状态精确匹配,而是**带命名寄存器的指令式语言**(实证 `lib/test_xlsx.py:274-314`):
  - **H 列(save_as)** = 把某步命令输出捕获进命名变量;**I 列(input_var)** / check_point 引用变量;
  - **check_point 的 found/not_found 可拿寄存器当 expect**(`found(寄存器, result)`);
  - **`execute`** = 通往任意 Python 的逃生舱(`lib/apv/apv_action.py` 注册 27 个 action,含投影 `提取*`、跨观测比较 `result_before != result_after` 等先例)。
- 全语料穷举(582 xlsx / 54436 行)实证:断言原语只有 `{found, not_found, abs_found, found_times}`(正则包含/计数);H 捕获 603 行、I 引用 1425 行、check_point 跟寄存器比 443 处(但全是"寄存器 vs 常量","寄存器 vs 寄存器"=0,因从没拼成捕获→互比)。
- **会话保持的正确编码**(纯现成原语):捕获首次 dig(`H=v1`)→ `found(v1)` 同池 / `sleep` → `not_found(v1)` 异池;dig 用 `+short` 去时间戳噪声。

## 8.9.2 重校准的数学模型(provenance **条件于 config**)

**§8.6 错在无条件算期望值的熵**,把"作者自己写进 config 的已知值"(池 IP 等)误判为"运行时不可知"。**作者拥有 config → 这些值是已知常量。** 熵应**条件于 config**算:

| 类 | 定义(条件于作者写的 config) | 编码 | ∈Cap(xlsx)? |
|---|---|---|---|
| **G** 文法 | 命令语法 | footprint/手册 grounding | ✅ |
| **E** 环境 | 拓扑 IP/可达性 | env_facts 查表(★ 可达列表) | ✅ |
| **V_K** 配置可推导值 | `H(v\|config,env,确定性语义)=0` → 作者算得出的常量(池 IP、超时、删后状态、rr 按序命中) | `found(常量,result)` | ✅ |
| **V_R** 运行时关系 | 断言是观测间关系(同/异) | **H 捕获 + found/not_found(寄存器,result)**,不预测不回填 | ✅ |
| **V_D** 设备生成不透明单值 | auto-gen 名/PID/时间戳 | `execute→提取` 或 `<RUNTIME>`+回填 | ✅ |
| **V_U** 真不可表达 | 需超出包含/计数且超出 execute 的 Python | — | ≈∅ |

判据:`断言可表达 ⟺ values(φ) ⊆ {V_K,V_R,V_D} ∧ 关系 ∈ {⊇,⊉,计数}∪Python`。老公式两错:①无条件算熵(误判 V_K 为不可知);②Cap(xlsx) 算少了(漏 V_R 捕获 + execute,只留 `<RUNTIME>`+回填,而占位只能填一个值、表达不了关系)。

## 8.9.3 纠错链(诚实记录:理论必须配执行验证)

会话保持可表达性,**连错多层,每层都靠更深一层的源码/gold/执行纠回**:
1. "xlsx 表达不了关系"——错(没读框架源码,只看自家产物只用 E/F/G)。
2. "有寄存器,`found(v1,v2)` 能表达相等"——错(没测 found 语义:它是子串包含非标量相等;整段 blob 比对因时间戳噪声恒失配)。
3. "缺投影+标量相等 2 原语,需 .py / 加函数"——过度设计(用户指出:**值是 config 已知常量、捕获是现成功能、`dig +short` 已去噪**)。
4. **最终**:现成原语 + 配置常量/捕获即可,**零新函数**。元教训:每次"算"不配对抗执行,就会停在似是而非的层级。

## 8.9.4 最小落地(已端到端验证)

| # | 改动 | 文件 | 治什么 |
|---|---|---|---|
| 1 | V_K/V_R/V_D 三分诊 | `agents/ist-compile-draft.md` Step 3 | draft 从 `<RUNTIME>`/静态析取 → 捕获+比较 |
| 2 | brief 决策树 | `compile_pipeline.py:_build_case_brief` | 同上,流水线侧 |
| 3 | emit 归一化(check_point 的 G 里是前序捕获变量名 → 移到 H) | `emit_xlsx_tool.py:_steps_to_caseir` | draft 把变量引用误放 G 列 → 确定性兜底移 H |
| 4 | env_facts ★ 列表解耦(先例为空时独立注入) | `compile_pipeline.py:_build_case_brief` | ★ 可达 IP 不再随先例消失 → 治"瞎编 IP" |

**验证(533097 端到端)**:draft 产出 listener `172.16.32.70`(★ 可达)+ `dig +short H=v1` 捕获 + `found(v1)/not_found(v1)`,**零 `<RUNTIME>`、零静态析取**;169 守护测试全绿。**纯 prompt + 两道确定性闸(零 emit 大改)就让 haiku 从"弃权/弱代理"切到正确捕获编码。**

## 8.9.5 "伪回归"教训(env_facts 耦合)

验证中 v2 draft 编了不可达 listener `172.16.35.100`,初判像"我改 prompt 引起的回归"。**实为既有 latent bug**:★ 可达 IP(确定性 env_facts)被 `compile_precedent` 捆进先例文本,先例为空(新意图/相似度<0.20 被跳过,如会话保持)时 ★ 随之消失 → draft 无 IP 锚点 → 探设备失败 20+ 次后瞎编(v2 日志冒烟证据)。先例=0 对会话保持是"一直如此",旧/v1 蒙中 34.70(运气)、v2 蒙到 35.100(不运气),**是无锚点掷骰子非回归**。已修(改动 4)。**教训:确定性事实(env_facts)绝不能耦合进非确定的检索(precedent),否则检索一空、确定性事实跟着丢。**

## 8.9.6 第二轮落地(2026-06-23 续:闭合 produce→grade 环 + 确定性兜底)

按计划 `plans/velvet-bouncing-riddle.md` 续做并验证:

- **A grade 捕获感知(交付必须,已落地)**:此前 draft 产对捕获,但 grade 会把它当弱断言 CUT(范式白做)——根因 `precedent_tools._load_case_rows` 不读 H 列、`confidence_f.build_judge_evidence` 把 `found(v1)` 渲染成 `found()`、`_JUDGE_SYS` 无关系断言概念、`compile_emit` docstring 仍写反范式。已修:① `_load_case_rows` 读 H;② `link_assertion_to_config` 带 `cp_h`/`capture_src`;③ `build_judge_evidence` **仅当有寄存器引用**改呈现(非捕获逐字不变,双层防回归);④ `_JUDGE_SYS` 加关系断言判据 + 防放水边界;⑤ grade.md/SKILL.md + emit docstring 同步。新增 `tests/case_compiler/test_confidence_f.py`(6 测,含防回归)。
- **C 可达闸 allowlist 兜底(已落地)**:`_gate_unreachable_listener` 原只拦"已知接口落盲段",补 allowlist——test_env 的 dig/curl 目标必须 ∈ ★ listener ∪ 后端 service,治"凭空编的可达子网内非 ★ IP"(如 35.100);空 ★ 降级放行。测试扩 2 例。
- **D IR 诚实(已落地)**:`provenance_ir._VALID_SOURCE_KINDS` 加 `config_derived`(V_K)/`captured_relation`(V_R);draft 捕获步标 `captured_relation`、配置常量步标 `config_derived`。下游全兼容(验证无破坏)。
- **回归**:全量守护 **177 测全绿**(原 169 + 新 8)。
- **端点切换(MiMo→DeepSeek)**:MiMo key quota 用尽(429),按用户指示切 DeepSeek(`OPENAI_BASE_URL`=api.deepseek.com/v1、key 拷自 `DEEPSEEK_API_KEY`、`IST_MODEL`=deepseek-v4-pro / `IST_HAIKU_MODEL`=deepseek-v4-flash;备份 `environment.bak.mimo` 可回滚)。探活 pro/flash 基本调用 + **函数调用**全通;速度比 MiMo 快(165s vs 243–596s/case)。

- **额外发现 + 修(draft 配置回显 padding)**:实测 grade(DeepSeek)对捕获断言**稳定给 1.0、识别为关系断言**(Phase A 生效),但 533097/545097 仍 CUT——根因 draft 给行为类用例多加了 `found(配置原文)` 配置回显断言(grade 正确判弱 0.x,`min()` 拖垮)。修:`ist-compile-draft.md` 红线加"行为/关系类用例别加配置回显断言(配置生效由后续 dig 隐含验证)"。

**B 子型泛化验证(DeepSeek 实测,5/5 端到端 PASS)**:

| 子型 | draft 产捕获 | grade overall | 端到端 |
|---|---|---|---|
| 533097 基础保持 | ✅ | **1.0 放行** | ✅ PASS |
| 545020 混合(有保持+无保持反向) | ✅ 同(found)/异(not_found)分诊都对 | **1.0 放行** | ✅ PASS |
| 545097 多类型优先级(配置回显修复后重跑) | ✅ 2 捕获变量(found×2+not_found×2)、回显0 | **1.0 放行** | ✅ PASS |
| 588691 clear session | ✅ found+not_found、回显0 | **1.0 放行** | ✅ PASS |
| 600249 删除中删配置 | ✅ found+not_found、回显0 | **1.0 放行** | ✅ PASS |

**结论:捕获范式 + grade 修复跨全部 5 个代表子型泛化、正反向(同/异)分诊都对、配置回显修复生效、端到端 PASS。** "xlsx 表达不了会话保持"**端到端证伪**——draft 产捕获 → emit/闸 → grade 放行 全链跑通,零新框架原语、零 `<RUNTIME>`、零 .py。

**仍未决**:上机真 PASS(需跳转机授权;注意多 IP 顺序稳定性、rr 首跳确定性两正交假设——这是 oracle 确定性,非表达力)。
