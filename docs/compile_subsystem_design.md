...
│                                                                                                       │
│  [版本闸] 目标产品+版本未提供 → qa_ask_user(不猜默认);版本 → 手册 glob(10.5_cli__part*.md)        │
│                                                                                                       │
│  ① prep                ② draft (fan-out 并发)        ③ grade (fan-out 并发)        ④ merge          │
│  qa_compile_prep   →   qa_compile_fanout         →   qa_compile_fanout         →   qa_emit_xlsx_merged│
...
│       ▼                      ▼         │                     ▼                        ▼             │
│  manifest.json          qa_emit_xlsx ──┤                qa_confidence_score      成品 case.xlsx      │
...
│                         · IP∈env_facts │   └────────────────┘  └──────────────────┘│               │
│                                        │   ┌─────────────────────────────────────┐ │               │
│                                        └───│ 先例检索 qa_lookup_pattern (无 embed) │─┘               │
│                                            └─────────────────────────────────────┘                 │
└───────────────────────────────────────────────────────────────────────────────────────────────────┘
                                                              │ 成品 excel(含 <RUNTIME> 占位)
                                                              ▼
┌──────────────────────── ist_verify skill (独立环节,串行上机) ─────────────────────────┐
│  qa_run_batch / qa_run_case(ist_compile_run fork,采 ground truth,不信 verdict 字符串) │
│       │                                                                               │
...
3. **不救场,诚实 escalate**:交付门槛是 grade 断言质量（弱断言 / 未覆盖 CUT 一律打回,不救场）;fail 如实记遗留,不替用例「想办法 pass」。连续 N 轮 CUT 则 escalate / `qa_ask_user` 上报,不闷头硬编码。
...
6. **版本先行**:不同产品版本对应不同 CLI 手册 / 语法。编译前必须确定目标产品 + 版本（从用户原文提取,缺则 `qa_ask_user`,**不从 config 猜默认**),据此推出手册 glob 写进每条 brief。
...
3. **G 列空壳 + 计划无上机**：只有步骤骨架，CLI 命令/断言列全空，从不调 `qa_run_case`。
...
- **零硬编码红线落地点**：`qa_compile_prep` 解析脑图 → manifest（`autoid` 为主键），但 case 的 `init_commands`/`steps`/`assertions_provenance` **全部为 null**——命令/断言由 draft 子 agent 现场查手册/先例回填。
...
- **变化**：编译链只产 excel（draft 生成 → grade **断言质量审批** → `qa_emit_xlsx_merged` 合并打包），**不上机**；上机独立成 `ist_verify` skill 对成品 excel 做。grade 的 device verdict 改**可选输入**，支持无上机静态审批。
- **交付门槛 = grade 断言质量**（弱断言/未覆盖仍 CUT，不救场），不再是上机 pass。两者经 `qa_ask_user` 在交互层串成闭环。
...
- **阶段一·补 意图检索轴**：`qa_lookup_pattern` 加可选入参 `intent=""`（默认空 = v1 行为不变，向后兼容）。原工具只按 my_config 命令 token Jaccard 检索，draft 必须先猜出配什么命令；676654 类一句话需求猜不出 → 检索不到 → 啃手册。加意图轴后"知道测什么但没想好配啥命令"也能检索到骨架先例。
- **E 段独立**：draft 直接调 `env_facts` 拿可达 IP，独立于先例通道——治论文 §5.6 点的"E 段搭载在 lookup_pattern 返回里、无独立查表通道"缺口。
- **结构约束门（correct-by-construction，命题 3.18）**：在 `qa_emit_xlsx` 出口做生成后过滤——命令头∈手册 allowlist、断言对象必须挂前序观测算子（非悬空）、绑定 IP∈env_facts 可达表。这是与 grade **独立**的确定性强制（grade 抓不到结构违反，两臂 grade-PASS 率同为 0.423 即证）。
...
| ② 三层 Provenance IR | §3.5 定义3.6/3.7 | `main/case_compiler/provenance_ir.py` + `qa_emit_xlsx` 加 `provenance_json=""` 入参 |
| ③ 意图族摊销 H_G | §3.7 定理3.10（同族 H_G 可共享） | `intent_cluster.py` + `qa_cluster_intents` |
| ④ 四层归因 | §5.4（G/E/V/瞬态四分） | `fail_attribution.py` + `qa_attribute_fail` |
...
**公开函数**：
- `emit_xlsx(file_ir, out_path)`（`xlsx_emit.py:62`）：编译链的产出端口（被 `qa_emit_xlsx` 工具调用）。流程：`shutil.copy` 模板 → `detect_xlsx_layout` 动态探测数据起始行（不写死 R29）→ 清空旧数据区 → 写 author 说明行(C=0) → 写 `init_rows`(C=1) → 逐 case 逐 step 逐 row 写（首行写 A/B/C/D，同步骤后续行只写 E-I，case 间空一行）→ `wb.save` → 返回 `_readback` 对账统计。
...
- `IdiomCorpus`（`corpus.py:213`）：懒加载内存索引。
  ...
  - `nearest(query, module, k)`（`corpus.py:284`）：token Jaccard + 模块加权最近邻检索，`qa_lookup_pattern` 的底层。
...
- `probe_show(command, build)`（`device_mcp_client.py:116`）：只读设备探针，经跳转机在被测 APV 跑单条 show/get（server 侧硬白名单首 token），取真实回显。本测试床 APV 只能经跳转机访问。`qa_probe_show` 工具底层。
...
**编译链定位小结**：`config`(事实源) 与 `object_normalizer`(归一规则) 是无依赖底座；`corpus` 依赖二者构建先例语料供 draft 检索（`qa_lookup_pattern`）；`case_ir`→`xlsx_emit` 是 draft 的产出路径（`qa_emit_xlsx`，emit 前过 `validate_case` 良构门、emit 后 round-trip 对账）；`provenance_ir` 是贯穿 draft/grade/verify/writeback 的来源契约；`confidence_f` 是 grade 的判分（`qa_confidence_score`）；`device_mcp_client` 是 verify 的上机通道，`runtime_fill`(回填锁死) + `verify_cache`(内容哈希跳过) 是 verify 链的两个确定性工具。
...
```
脑图 txt
  │  qa_compile_prep              （compile_prep.py）—— 只产需求，零命令
  ▼
manifest.json  { cases:[{autoid,title,group_path,step_intents,…null…}], groups }
  │  qa_compile_pipeline._run_pipeline  （compile_pipeline.py）—— 确定性锁死全序列
  ...
  ▼
grade-PASS 的 case
  │  qa_emit_xlsx_merged          （emit_xlsx_tool.py，经 _load_case_rows 读回 steps）
  ▼
workspace/outputs/<out_name>/case.xlsx   （N 真 case + 1 哨兵；不上机）
```

`batch_tools.py` 的 `qa_compile_fanout` / `qa_run_batch` 是更早期/外部可复用的阶段工具；`qa_compile_pipeline` 现在把 draft/grade 的 fan-out **内联**进 `_run_pipeline`（直接调 `execute_fork_skill` + `AdaptiveLimiter`），不再走 `qa_compile_fanout`。上机验证（`qa_run_batch`）属独立 `ist_verify` 环节，编译流水线**不上机**。
...
- **`qa_compile_prep(mindmap_path, out_name="") -> str`** (`compile_prep.py:97`，`@tool`)：
...
- **`qa_compile_pipeline(mindmap_path, product_version, out_name="") -> str`** (`compile_pipeline.py:495`，`@tool`)：
  - **作用**：一次跑完一个脑图的全编译序列，产出合并 `case.xlsx`。**整个编译子系统对主 agent 暴露的唯一编排入口**。
  - **关键入参**：`product_version` 不可臆测（决定查哪版手册），缺失直接返回 error 提示先 `qa_ask_user`；`out_name` 空则用脑图 stem，并 `replace("/","_")` 防路径注入。
...
- **`_run_pipeline(mindmap_path, product_version, out_name, *, draft_skill, grade_skill) -> dict`** (`compile_pipeline.py:346`)：确定性跑完全程，错误进 `result["errors"]` 不抛。三段：
  1. **prep（一次）**：调 `qa_compile_prep`，读回 `manifest.json` 拿 `cases` + `groups`；无 manifest / 无 case 即提前返回。
  ...
  3. **merge（一次）**：对每个 done case 用 `_load_case_rows(xlsx)`（`precedent_tools.py:133`）读回 steps，组装 `merged_cases`，调 `qa_emit_xlsx_merged`（`emit_xlsx_tool.py:494`）合并成单 excel + 尾部哨兵。
...
- **`_parse_grade_verdict(out) -> bool`** (`compile_pipeline.py:330`)：取**最后出现**的裁定词（PASS vs CUT 的 `rfind` 比位置）——grade 报告会先讨论 `qa_confidence_score` 的分（可能含 CUT）再下结论，朴素的 `"PASS" in out and "CUT" not in out` 会把"工具说 CUT 但我判 PASS"误读成重做，造成无谓 churn。`ERROR:` 开头一律不通过。
...
- **`qa_compile_fanout(skill, briefs_json, concurrency=0) -> str`** (`batch_tools.py:71`，`@tool`)：
...
- **`qa_run_batch(xlsx_path, autoids_json, module="", build="", max_s_each=600) -> str`** (`batch_tools.py:163`，`@tool`)：
  - **作用**：把一个合并 xlsx 里多个 case **顺序上机**，逐个回 verdict + 框架真实裁决。属 `ist_verify` 上机环节（**不在编译流水线内**）。
  - **串行硬约束**：跳转机框架有全局运行锁（`run_and_wait` 拿不到 task_id 即"submit failed (lock held?)"）+ 设备配置/统计全局共享态——内部就是一条 SSH 会话的 for 循环，**物理上不可能并发上机**。与 `qa_run_case` 区别：跑同一 xlsx 的一批 autoid，省 N-1 次 SSH 连接开销。
...
- **`qa_emit_xlsx_merged(cases_json, shared_init="", out_name="") -> str`** (`emit_xlsx_tool.py:494`，`@tool`)：
...
- **编译/上机解耦**：编译流水线**不上机**（`qa_compile_pipeline` 只产 excel）；`qa_run_batch` 属独立 `ist_verify` 环节，运行时值留 `<RUNTIME>` 待上机回填。
...
- `/Users/jiangyongze/Library/CloudStorage/SynologyDrive-home/Project/InfoTest_Engine/main/ist_core/tools/device/emit_xlsx_tool.py`（`qa_emit_xlsx_merged`）
...
编译链的最后一道关卡。draft 子流程查手册/先例后产出步骤列表,通过 `qa_emit_xlsx` / `qa_emit_xlsx_merged` 落地为框架原生 `case.xlsx`。这两个工具不是简单的格式转换器,而是**确定性闸门集合的执行点**:在产出 xlsx 之前,串行跑过一组 `_gate_*` 与可选的结构门,任何一道挡住即打回 draft 并附带可执行的修正指引。
...
- **位置**:`ist_compile_batch` 编排的 `qa_emit_xlsx_merged` 合并阶段,fan-out 出 N 个 case 后的打包动作。
...
#### `qa_lookup_pattern(my_config, limit=3, intent="")` → str（`precedent_tools.py:174`）
...
#### `qa_confidence_score(xlsx_path, need_intent="", anchor_examples="", manual_facts="")` → str（`precedent_tools.py:272`）

- **作用**：判 case.xlsx 的断言「配不配得上它所测的配置行为」，给 CUT/PASS 决策。这是**第二判据，不是 verdict**（`:277-278`）——verdict 看「能不能跑通」，f() 看「断言有没有咬住需求要测的行为」。
- **关键入参**：`anchor_examples`（`qa_lookup_pattern` 返回的先例文本，判分锚点，强烈建议填）+ `manual_facts`（grep 手册得到的「该配置该产生什么可观测行为」）——证据越全判得越准。
...
#### `qa_run_case(xlsx_path, autoid, module="", build="", max_s=600)` → str（`run_case.py:30`）
...
#### `qa_probe_show(command)` → str（`run_case.py:182`）

- **作用**：经跳转机在被测 APV 上跑**单条只读 show/get 命令**，取真实回显。硬白名单：首 token 必须 `show`/`get`（`run_case.py:217`）。
- **拓扑约束（`run_case.py:184-188`）**：APV 只能经跳转机访问，`qa_ssh`/`qa_restapi` 直打 APV IP 不通——是拓扑使然非掉线。
...
#### `qa_list_runtime_slots(xlsx_path)` → str（`runtime_fill_tools.py:39`）
...
#### `qa_fill_runtime(xlsx_path, fills_json, run_meta="")` → str（`runtime_fill_tools.py:71`）
...
- **关键入参**：`fills_json`（JSON 数组，每项 `slot_id`/`runtime_value`/可选 `evidence`）。值必须来自设备真实输出（`qa_run_batch`/`qa_run_case`/`qa_probe_show`），**抽不出就给空值＝如实留空，绝不猜一个**（`runtime_fill_tools.py:79-81`）。`run_meta` 写入 provenance 溯源。
- **返回**：`filled` / `left_blank(如实留空)` / `not_found(已锁死/id 错)` 各 slot_id + 明细 + 仍待回填槽位数。
- **底层**：`case_compiler.runtime_fill.apply_fills`。
- **链中位置 / 被谁调用**：ist-fill fork——先 `qa_run_batch`/`qa_run_case` 拿设备输出，再 `qa_list_runtime_slots` 看待填槽位，最后 `qa_fill_runtime` 锁死写入（`runtime_fill_tools.py:3-8`）。这是「编译留 `<RUNTIME>` 空 → 上机回填真值」解耦的回填半环。
...
- **`qa_attribute_fail(...)`** → str（`fail_attribution.py:99`）：tool 包装，返回 JSON `{layer,reason,reflow,target_layer,render}`。供 verify 子流程对每个 fail 先确定性初分、再人工核对语义。
...
- **编译产 excel / ist_verify 上机回填解耦**：编译链（draft→grade）只产 excel，离线不可知的运行时值由 draft 诚实留 `<RUNTIME>`（5.1 检索 + 5.3 列槽）；`ist_verify` 独立对成品 excel 上机（5.2 `qa_run_case`）采真值后 `qa_fill_runtime` 锁死回填（5.3）。两半通过 `<RUNTIME>` 占位符 + 结构性锁衔接，互不污染。
- **真机裁决不信 verdict 字符串**：`qa_run_case` 显式以框架 MySQL `Success/Fail Num` 为唯一裁决，主动抽因果行高亮、原样回执行明细 ground truth，并文档化警告 `=== 1 passed ===` 不代表断言通过（`run_case.py:53-57/146-170`）。
...
- **`_SYSTEM_PROMPT`**（`extractor.py:20`）— 提取契约。约束 LLM：cli_syntax 必须是**完整规范签名**（综合参数表还原，不照抄残片、不把示例具体值当命令 token）；`no/show/clear/配置` 四态各自提取（feature_path 相同会自动归一节点）；evidence_quote 必须是原文片段（merge 会 grep 验证）；架构名词/项目代号不收；known_issue 的 issue_title 照抄 `web_bug_search` 原文。
...
一句话职责：从 `nodes/*.json` 懒加载，建 dict 索引，提供精确查找 / 模糊搜索 / 统计，是 `qa_footprint_lookup` 与 reminder 自动注入的共用后端。进程级单例。
...
- **`current_arm()`**（`ablation.py:26`）/ **`is_baseline()`**（`ablation.py:32`）/ **`arm_tag()`**（`ablation.py:37`）— 读 env `IST_ABLATION_ARM`，非 `'E'` 一律按 `'L'`。Arm-E 在三注入点分叉：G 段 `qa_lookup_pattern` 不返先例、E 段 `qa_emit_xlsx` 跳过可达校验门、V 段 `qa_confidence_score` 直接放行。设计红线：默认 L、只读 env 不写状态、每次现读便于同进程切臂跑对照。
...
- **`TOOL_METADATA`**（`metadata.py:31`）— 字段语义：`read_only`/`concurrency_safe`/`fallback_for`/`intent`。`qa_footprint_lookup`（`metadata.py:187`）声明 `read_only=True, concurrency_safe=True, intent="search"`；编译链的 `qa_lookup_pattern`/`qa_confidence_score`/`qa_emit_xlsx`/`qa_compile_*` 等同在此表。
...
读取链（编译链内，给 draft grounding）：`qa_footprint_lookup`（agent 主动按命令名查，带模块总开关上溯）+ reminder 自动注入（`MemoryInjectionMiddleware` 走 `FootprintIndex`）→ 供命令语法/规则/缺陷；并行 `env_facts.summary_for_agent()` 供 IP 真值。两者共同构成 draft "correct-by-construction" 的事实供给，配 merge 的 60% evidence 门与 emit 的可达校验门实现"不救场、零硬编码"。
...
把"主 agent 编排器 → 派发 fork 子流程"这件事落到代码。核心机制三件：①SKILL.md/agent.md 的双文件加载（`loader.py`）；②`qa_invoke_skill`/`qa_compile_fanout` 把任务文本派发进隔离 fresh subagent；③渐进披露 + 零写死命令红线。设计意图是用**进程级隔离**消除"自生成自评估"——draft 生成、grade 审批跑在彼此看不见、各自全新上下文的 subagent 里。
...
- **`_get_tool_registry() -> dict`** / `_TOOL_REGISTRY`（`loader.py:142-219`）——延迟加载的工具白名单。fork subagent 只能用注册表里的工具，frontmatter `tools:` 里写的名字必须在此命中才会被 `_resolve_tools`（`loader.py:262-282`）解析成真实工具对象，命不中静默丢弃。注册的编译相关工具：`qa_lookup_pattern`/`qa_confidence_score`（先例与判分，`loader.py:210-216`）、`qa_emit_xlsx`/`qa_run_case`/`qa_probe_show`（单 case，`loader.py:184-194`）、`qa_compile_prep`/`qa_emit_xlsx_merged`/`qa_compile_fanout`/`qa_run_batch`（批量编排，`loader.py:196-208`）、`qa_footprint_lookup`、`qa_deepagent_grep/read_file/ls` 等。**这是 fork 子 agent 的能力边界硬约束**——draft 拿不到 grade 的判分工具、grade 拿不到 emit/run 工具（见 7.4），靠 frontmatter `tools:` + 注册表双重过滤实现。

- **可观测性三函数**（`loader.py:45-133`）：fork 内部 LLM 往返不进主 stream，故每次 fork 执行落两份日志。`_summarize_fork_messages`（`loader.py:73`）统计工具调用分布/AI 轮数/ToolMessage 数，并抽出 `qa_deepagent_grep`/`qa_footprint_lookup` 的实际查询词（诊断"在查什么"）；`_trace_fork`（`loader.py:107`）写人读摘要到 `runtime/logs/fork_trace.log`；`_record_fork_status`（`loader.py:45`）写机读 JSONL 到 `runtime/logs/fork_status.jsonl`（崩溃时"哪些 fork 已完成、产出什么"不丢）。三者失败一律静默，绝不挂主流程（`loader.py:69`、`132`）。

- **渐进披露辅助**：`read_skill_frontmatter`（`loader.py:240`）、`skill_disable_model_invocation`（`loader.py:235`，`disable-model-invocation: true` 禁止主 agent 经 `qa_invoke_skill` 加载）、`list_subagents`（`loader.py:374`）。

### 7.2 派发入口：`qa_invoke_skill` 与 `qa_compile_fanout`

- **`qa_invoke_skill(skill, brief)`**（`tools/skills/__init__.py:25`）——主 agent 调技能的统一入口。校验技能名合法性、存在性、`is_callable_by_model`（渐进披露状态门，`__init__.py:58-65`）、`disable-model-invocation`（`__init__.py:72`）后分流：`context: fork` → `execute_fork_skill`（`__init__.py:79-81`）；inline → 直接返回 SKILL.md 文本注入主对话（`__init__.py:84`）。`docstring` 里的 **BLOCKING REQUIREMENT**（`__init__.py:32-34`）是渐进披露的措辞约束：技能描述命中用户请求时，必须先调本工具再做别的。

- **`qa_compile_fanout(skill, briefs_json, concurrency=0)`**（`tools/device/batch_tools.py:72`）——批量并发派发。把 `briefs_json`（`[{"key":..., "brief":...}]` 数组）的每条 brief 通过 `execute_fork_skill(skill, item["brief"])` 在线程池里并发执行（`batch_tools.py:116-125`），返回每个 key 的产物。设计意图：比逐个 `qa_invoke_skill` 串行快 N 倍且各 fork 互相隔离不串话（`batch_tools.py:77`）。**每条 brief 就是本来要传给 qa_invoke_skill 的那段文本**（`batch_tools.py:83`）——fanout 只是并发包装，不改变单条派发语义。这是编排器把 draft/grade 阶段并发铺开的物理手段。

### 7.3 编排层四个 SKILL.md 的职责

| skill | context | 入口性 | 职责 | 关键约束 |
|---|---|---|---|---|
| `ist_compile_batch` | inline | user-invocable | **唯一编译入口/编排器**：通读用例→`qa_compile_prep` 解析 manifest→`qa_compile_fanout(draft)` 并发生成→`qa_compile_fanout(grade)` 并发审批→`qa_emit_xlsx_merged` 合并打包。单条是 N=1 特例同流程。 | 不上机（解耦）、编排器步数纪律、零命令进 brief |
| `ist_compile_draft` | fork (`agent: ist-compile-draft`) | 否 | 把一条用例生成单 case xlsx 草稿：核查前置→检索先例→`qa_emit_xlsx`。 | 不上机、不自评、IP 只用测试床真实可达值 |
| `ist_compile_grade` | fork (`agent: ist-compile-grade`) | 否 | 独立审批 draft 断言是否真覆盖目标行为：`qa_confidence_score` → PASS/CUT + 重做意见。 | 只读、不生成不上机、审批不依赖上机裁决 |
| `ist_verify_v3` | inline | user-invocable | 对成品 excel 串行上机验证 v3 链：上机回填 `<RUNTIME>`→采集裁决→`qa_attribute_fail` 四层归因(G/E/V/瞬态)按层回流→上机真 PASS 写回 footprint。 | 只验不生成、上机串行、只写回 G 段 |
...
- **编译与上机验证解耦**（2026-06-16，`SKILL.md:38-39`、`134-137`）：本 skill 只产 excel，流程内**不调** `qa_run_batch`/`ist_compile_run`；上机走独立 `ist_verify`。原因：上机大面积失败常是设备环境瞬态（SSH 中断/dig 超时/DNS 失败），不该阻塞产出。
...
- **不救场+不空转**（`SKILL.md:88`、`94`）：合并哪些/要不要重做是编排器自主决策，**绝不 qa_ask_user 问用户**；部分 PASS 部分 CUT 是常态（PASS 先合并、CUT 另行重做），非交互模式尤其不能卡在 ask_user 空转（撞 ask_user error 反复 ls/ask 转圈会被 loop guard 拦截）。
...
- **版本校验前置**（`SKILL.md:49-54`）：缺版本立即 `qa_ask_user`，不猜默认值；版本推出对版本手册 glob（`10.5` → `10.5_cli__part*.md`，禁用通配以免命中别版本）写进每条 brief 的指路。

**`ist_verify_v3` 要点**：相对 v2 三个增强（`SKILL.md:21-32`）——①上机回填 `<RUNTIME>`（draft 对离线不可知值诚实留占位、不猜；verify 用设备真实输出填且**锁死不反复改**，`qa_fill_runtime` 只动仍含 `<RUNTIME>` 的格子）；②四层归因（`qa_attribute_fail` 把每个 fail 归 G/E/V/瞬态，按层定向回流比整条打回省）；③闭环写回（上机真 PASS 的 G 段文法写回 footprint，evidence 门防幻觉，V 段断言/E 段具体 IP 不写回以免污染，`SKILL.md:39`、`82`）。红线：裁决以框架逐 check_point 真实明细为准不信 verdict 字符串（`SKILL.md:36`）、归因如实不救场（`SKILL.md:38`）、上机串行只走 `qa_run_batch` 不并发（`SKILL.md:40`）。

### 7.4 两个 fork 子 agent 的角色与装配纪律

**`ist-compile-draft`（`agents/ist-compile-draft.md`）**——草稿生成子流程。

- frontmatter（`md:1-7`）：`tools: qa_lookup_pattern, qa_emit_xlsx, qa_probe_show, qa_deepagent_grep, qa_deepagent_read_file, qa_footprint_lookup`；`model: opus`；`inherit-parent-prompt: true`。**工具集里没有 `qa_run_case`（不上机）、没有 `qa_confidence_score`（不自评）**——能力边界靠工具白名单物理切断，不靠提示词自律。
- 装配纪律（`md:9`、`19-37`）：核查前置（`qa_lookup_pattern` 取先例**完整 init** 完整沿用、`qa_probe_show` 查设备现状、grep **对版本**手册）→检索先例确定测法→`qa_emit_xlsx` 生成。红线：**CLI 文档是唯一权威**（`md:34`，未明确定义的参数标"未生成"不推断）；**期望值溯源**不 observe-then-assert（`md:35`）；**不自评不上机**（`md:36`）；**IP 只用测试床真实可达值**，绝不照抄先例/手册示例 IP（1.1.1.1/10.x 等不可达，dig 必失败 Hit=0；`qa_emit_xlsx` 出口按事实源校验打回，`draft/SKILL.md:16`）。

**`ist-compile-grade`（`agents/ist-compile-grade.md`）**——质量评估子流程。

- frontmatter（`md:1-7`）：`tools: qa_confidence_score, qa_lookup_pattern, qa_deepagent_grep, qa_deepagent_read_file`；`model: opus`；`inherit-parent-prompt: true`。**只读工具集，没有 `qa_emit_xlsx`/`qa_run_case`**——物理上无法生成或上机，只能判分。
- 装配纪律（`md:9-41`）：判据是**断言覆盖度**这一与"能否跑通"正交的维度（`md:13-16`，即便跑通弱断言也判 CUT，这正是本子流程存在意义）；`qa_confidence_score` 判分 + **对抗性独立核对**（不只依赖判分，对照需求核心动态行为/对照先例，`md:28-32`）；CUT 必给**具体到可据以修改**的重做意见（哪条弱/为什么/参照哪个先例改成何形态，`md:35`）。红线：**不自评不重做**（评估对象是生成子流程产物非自身，修改属生成子流程，`md:38`）；**审批不卡上机**（环境能否跑通是 ist_verify 的事，`md:39`）；每个"此条弱"判断须引用 xlsx 行号+需求原文+先例/手册出处（`md:41`）。
...
2. **零写死 sdns/APV 命令**：编排器 brief 五要素 + 自检三问、draft "CLI 文档唯一权威"、grade "不硬编码命令"——任何具体设备命令都只能由子 agent 现场 grep 对版本手册/`qa_lookup_pattern` 查先例得出，进 brief/manifest 即视为滑回被推翻的硬编码老路。
...
- `/Users/jiangyongze/Library/CloudStorage/SynologyDrive-home/Project/InfoTest_Engine/main/ist_core/tools/skills/__init__.py`（`qa_invoke_skill`）
- `/Users/jiangyongze/Library/CloudStorage/SynologyDrive-home/Project/InfoTest_Engine/main/ist_core/tools/device/batch_tools.py`（`qa_compile_fanout`）
