...
## zhaiyq 重跑新面板续记（2026-07-18，HEAD 644a0b26）

**正面复验（修复生效）**：
- ✅ **D2 用户可判断性**（最严重缺陷）：zhaiyq 面板选项全带「对你的用例：…」人话后果（如「承认实机对、改用例期望」「判定产品问题、不改用例」）——F-LLM-1 生效，普通用户可判断。
- ✅ **D1 英文泄漏**：面板全中文，冲突讲透（双方记载+情况梳理+核心问题），未复现。
- ✅ **升级人工路径实弹**（leader 入正面册）：P2-o 确认产品缺陷路径措辞清楚（「记进缺陷候选清单、这个用例按缺陷结案」），545097 走通、缺陷候选正确入册。
- ✅ **行为知识晋升 10 条**（leader 入正面册）：自愈 footprint 写回工作——zhaiyq 冷启动域开始积累判例（会话保持 ALL 合法性、per-record timeout 等），是下批同域提效的种子。判例采纳机制（588990 采纳 589432 先例）实证 shape-aware 采信正确。

**新发现缺陷**：
- **D22【P2·题面原样嵌手册内容（文件名+md 标记两症）】**（leader 登记，Py-Eng 收口批①族）：s0_dispute 面板「双方记载」段原样嵌内部内容——**子症 Z1（文件名）**：`attr_evidence.json`、`cli_10.5_Chapter20.md L529` 内部文件名/行号直抛用户，attr_evidence.json 出现两次（两证据同名分不清）；**子症 Z7（md 标记）**：手册引文的 `**`/`_.._` markdown 标记字面泄进题面（原样 copy 手册 md 内容）。**同根=原样嵌手册内容**，修法同处：证据源人话化+两证据分名（「设备实测」「用例步骤N 记录」）+ `_strip_md(quote)` 机械剥 markdown 标记（设备回显不剥）。归 questions.py（P2）。
- **D23【P2·多题面板 hint 文案-行为分叉】**（归 TUI-Eng；**我初判"数字键切题异常"经 TUI-Eng 机读更正**）：真相=`send '2'` 其实**落答 option 2 + 自动前进**（数字直选=落答+前进的 B 行为，P1-5 修复时定），**数字键本身可用且防丢答（选完即落）**。缺陷=底部 hint「↑↓/数字 移动」停留在旧 A 叙事（应为「数字=选中并进下题」），文案-行为分叉致我误判"没选中"。幸 G4 echo 验证兜住（send '2' 落的恰是要的②）、无实害。修法=hint 文案对齐行为，收口批落。**后续数字键可继续用**（N=选中 option N+进下题）。
- **D24【P2·多题面板回扫显示光标重置】**（我 panel 实操发现，归 TUI-Eng）：←→ 导航回退时 ❯ 重置到默认 option 1（内部选择保留、但视觉误导无法目视核实已选）。规避=从首题干净重做前向 pass 逐题 verify 高亮再 enter。加剧丢答风险。
- **D25【P3·diagnose 状态行 s₀ 黑话堆叠】**（leader 登记，低优，Py-Eng ①族顺手修）：diagnose 状态行 `fork 判 s₀ 但机械配对判无污染者——不升格,保留深归因`——s₀/污染者/升格/深归因四内部术语堆叠 user-facing 不可解。emit 瞬态不阻塞、边缘豁免，修法=人话化一句（「疑似受前案影响但未证实,继续深查」）。
- **D26【P2·正文交叉引用裸 autoid】**（leader 登记，User 挑 588990 面板，Py-Eng ①族）：面板正文「同批 case **205271757988589432** 已有相同争议的先例裁决」——直接抛 18 位裸 autoid 给用户（无人话/无尾号简写）。修法=交叉引用先例案用人话/尾号（如「同批尾号 9432 案」）。归 questions.py 文案层。
- **D27【P3·footer 数字核验后回退（低优观察项，不强制修）】**（leader 登记裁定）：footer 40/53→37/53 数字回退——子集核验把 3 案乐观产出重分类，**是入账核验的正确行为**，但用户视觉「怎么倒退」困惑。**裁：留档观察不强制修**——#27 修复后编写期/入账期字段分离会让语义更清晰、数字回退是正确表现，过度标注反复杂；zhaiyq 后 User 复提再升级。收口批不占位。
- **D28【P1·cap 面板 fail 原因英文+截断】**（leader 登记，Py-Eng ①族，Design 出渲染侧中文摘要形态）：cap/轮次封顶面板（545249）三轮 fail 原因**直灌英文 attribution + 截断**——「pyATS Errored (protocol-hard signal: assertion-window distortion…」「device rejected at p2: after the first pool name the de…」「After timeout cleared the session…」。**D1 英文泄漏+D9 截断在 cap 路径复现**（dispute 面板清扫没覆盖 cap 的 attribution 透传；语言分层：归因 reason 英文是 LLM-facing 合规，但进用户面板须中文转述=渲染层职责）。影响=用户无法据 fail 原因判断"继续/挂起/停止"。归 Py-Eng ①族。**键名定 `user_note`**（LLM-Eng 亲核发现消费链早已备好该复用预留字段——渲染侧填 user_note 即可，收口批工作量半份再缩）。
- **D29【P2·cap 面板同面板数字不一致】**（leader 登记，User+我双视角，D17 族，Py-Eng 查计数口径）：cap 面板正文「[引擎已试:**重编 2 次**] 已**重编 3 轮**仍未通过」——「重编 2 次」与「重编 3 轮」同面板两个计数不一致（差 1，疑首轮编写 vs reflow 轮次口径混）。**我快评时读到这两个数字却没交叉核一致性**（同 D16 号码形态 miss，四标准快评须补"同面板数字一致性"检查项）。归 questions.py 计数口径统一。

**正解交互模型固化**（供后续面板复用，2026-07-18 经 TUI-Eng 更正）：**数字键 N=选中 option N 并自动前进下一题（select+advance，防丢答，可用）**；enter=选定当前高亮 option 并前进；末题选定=提交；单题面板无 Tab 切题、选定直接提交；箭头键 ↓↑ 移 option、←→ 切题。**D24 修前回扫显示光标重置（内部选择保留、视觉误导）**，修后光标指已选项。核实靠 G4 echo（引擎回显裁决）最可靠。zhaiyq 首面板（s0_dispute 3 题 ②①①，我当时误判改走箭头键稳妥法、实则数字键也对）+ 第2面板（采纳先例①）均 G4 echo 验证零丢答。
