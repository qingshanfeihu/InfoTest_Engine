...
## D. team3 线3 追加(2026-07-16 · §2.1 (a) 表非热区条目缩写轮)

### D1 · DNS 标签 lint 检测面双闸设计(缩写前原文;DESIGN §2 现措辞未覆盖)
**源**:`main/ist_core/tools/device/structural_gate.py:1097-1102`(缩写前) · **建议归入**:DESIGN_dongkl_finalization §2
**归档理由**:DESIGN §2 现文描述为「裸标签扫描(`dig `/`host name ` 行内任意 `[A-Za-z0-9-]` 连续段 >63 即违例)」——
那是规划期朴素形态;实现刻意**不是**任意长串扫描(那正是强字典误杀,GA-CUT 型),而是双闸限定到域名 token。
文档措辞与实现的这一差异 code_align D2 已记名字回填、但检测面描述仍未回填——按「未覆盖先归档再缩」纪律抄录:
```
...
```
*价值*:双闸检测面(承载命令位×纯域名 token)+ GA-CUT 误杀规避判据 + §18.15-A S4 覆盖清单(host pool/hostname/续行)。
*处置*:源处已缩(RFC 63 契约首行保留+指针);全部 why 同时承载于紧邻 `_check_dns_label_limit` docstring(行为契约,永久 KEEP)。
**建议文档 owner**:DESIGN §2 检测面措辞按上文回填(与 D2 名字回填同源,现文「任意连续段>63」若被照做即重蹈 GA-CUT)。
