...
- **M 计算方法（防照模板填数者重新发明；Py-Eng 裁定）**：M = 脑图 `title`/`steps` **正则抽候选缺陷号** → `kb_bug_search` enrich **逐个辨伪**（查得到=真 ticket 计入、查不到=误抽滤掉）→ 去重 + 按 `group_path` 归组。**守 C1（结构化非强字典）**：正则抽号（结构锚）+ ticket 实查辨伪，**不做语义匹配**（防强字典误判、同 GA-CUT 教训）。**K 不可得时标「基线待补」**（M 单独成立、不阻塞交付）——kb_bug_search 是 ticket 查非搜索，case 不自带 bug 引用故 M 靠正则抽+辨伪，非现成 linkage 源。
