# de-escalate 通道修法规格（草案 v1，leader 起草待团队评审）

> 2026-07-20，用户裁 B-1 走 A（修缺口）后 leader 起草。前置取证见 `team4_escalated_deadlock.md`。
> 状态：**草案，待 Theory/Design 语义会签 + Py-Eng/TUI-Eng 实现认领**。理论/设计正文零改动纪律照守。
...
**恢复出口 = A6 三分（Theory 必改，leader 采纳收回原裁）**：恢复不是二选一「实然手段/缺陷候选」，THEORY:273 A6 已给三个出口——
| 出口 | 语义 | 适用 |
|---|---|---|
| 实然手段（重编/换床复跑） | 还有没试尽的实然路 | no_output / not_executed / no_ledger_channel 的因① |
| 产品缺陷候选（Ω③，THEORY:102 定义域：产品缺陷·四查+换形态坐实） | 案侧无修法的**产品**问题 | 四查坐实后 |
| **工程故障呈报（A6 臂）** | **引擎**缺口，附证据，**不进缺陷候选卷** | no_ledger_channel 的因②（先重编试探后坐实） |
> **裁决更正留痕**：leader 初裁「no_ledger_channel 统一转缺陷候选」，经 Theory 依 THEORY:102 定义域（引擎缺口非产品缺陷、过不了四查、无形态可换）+ A6 既有二分驳回，Py-Eng 独立佐证（or 分类不可分、先试后判）——**leader 收回原裁**。缺陷候选卷若日后出现引擎条目可溯回此处。
