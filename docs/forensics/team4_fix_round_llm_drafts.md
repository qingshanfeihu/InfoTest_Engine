...
- **第四应用面(设计侧,#44 §1.3.3「必留人工」实证,Design 2026-07-19 点)**:给机械不可判的面加机械默认 = 让"量不到满射"的信号膨胀成"覆盖是否完整"的判决 = over-mechanize。防它的手段=自由度分层留「必留人工」极(测不准的面别加机械 gate)。**Design 观察:设计评审门 discipline(信号只授权它实际量到的)与自由度分层(别给测不准面加机械默认)在此 meet 成一条**——同一膨胀-避免原则的评审侧与设计侧。四应用面(亲核指向 / 读 scope 阅读 / 信号证据 / over-mechanize 设计)同一根:**局部授权别膨胀成没授权的范围**。
