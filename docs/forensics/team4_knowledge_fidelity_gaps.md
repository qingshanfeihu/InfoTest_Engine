...
## `_append_cli_command` 出口分支表（S2 落地面）

> **⚠ 标题已于 v2.4 更正（Theory 异议①）**：本表原标「A2″ 三分条款 → 代码面映射」，**是错的** —— 本表列的是**控制流出口**（同语法 / 挂接 / 新建），A2″ 三分是**字段轴**（同键再到达时每个字段怎么处置）。两者恰好都是 3，但**正交**；照原标题读会得出「cli 路已落地 A2″」的错误结论。**A2″ 的真代码面表见 v2.4 微正节。**

`_append_cli_command`（`merger.py:314-403`）现有**三个出口**：
...
# v2.4 微正：A2″ 换轴错位 + 条件甲基准（Theory 异议 · 2026-07-20 · 终）

Theory 对 v2.2 提两异议：**其二已由 v2.3 解决**（观察组二分，`:931-947`，已确认）；**其一成立**，本节微正。

## 异议①：映射表同名不同轴（成立，已改标）

- v2.2 的「A2″ 三分条款 → 代码面映射」表，列的是 `_append_cli_command` 的**三个控制流出口**（同语法 / 挂接 / 新建）；
- 而 A2″ 三分是**字段轴** —— 同键再到达时，**每个字段**怎么处置（身份取既有 / 状态不降级 / 其余累积）；
- 两者**恰好都是 3，但正交**。照原标题读，实现方会得出「cli 路已落地 A2″」的结论，而**那里的证据字段一条累积都没有**。
- 处置：原表标题已改为「`_append_cli_command` 出口分支表（S2 落地面）」并加了指向本节的警示（见 `:876`）。

## A2″ 真代码面表：三个 handler 的同键字段处置现状

| handler | 身份字段（取既有） | 状态字段（不降级） | 其余字段（累积） |
|---|---|---|---|
| `_append_behavior` | **✓** `fact_key` 取既有 | **✓** `validity` 单向 `uncertain→verified`、反向不降级 | **✗** `observed_under` **覆盖**（`:434-435`）、`evidence` 覆盖 |
| `_append_cli_command` | **✓** `command` 不改（签名以手册为准） | n/a | **部分 ✓ / 部分 ✗**（见下） |
| `_append_decision_rule` | **✓** | n/a | **✗ 全无** —— 同键即 `return "skip"`，一个字段都不动 |

**结论与 Theory 一致：A2″ 实现 = 三个 handler 全改，不止 `_append_behavior` 两点。**

### 一处细节修正（对实现有用，请转 Theory）

Theory 表中 `cli_command` 记「累积全 ✗」，**不完全准确**。核实后是**一半一半**：

- **`parameters` 已是累积，且形态正确** —— `_merge_parameters`（`:288-308`）：按 `name` 合并，新增缺失参数、**补全已有参数的空字段、不覆盖已有非空值**，返回是否有变更。
- **`evidence.device_run` 是覆盖** —— `_attach_device_run`（`:336-353`）只保留最近一次 `(autoid, run_ts, build)`；其 docstring **实现者自标**：「多次实证的语境累积（同命令在不同 build/配置下的历次运行）属 R2 语境轴，**未做**」。

**这对 A2″ 实现是好消息**：`_merge_parameters` 的「**补全而非覆盖**」正是 A2″ 想要的累积语义，且已在生产中跑了很久 —— **A2″ 的累积改造可直接复用其模式，不必从零设计**。同时 `_attach_device_run` 的 docstring 已经把缺口标在原地，是天然的落点锚。

## 异议②：条件甲基准钉死

条件甲原文「`decision_rules` 按 `_append_behavior` **同形**改造」存在基准歧义，**钉死如下**：

> **基准 = A2″ 改造后的 `_append_behavior`，不是当前版。**

理由：当前版的 `_append_behavior` 自身带 `observed_under` **覆盖病**（`:434-435`，即 P1-8/C3 要治的那条）。**照当前版同形，会把覆盖病一并抄进 `decision_rules`** —— 修了一个 section、同时给另一个 section 种上同样的病。

**实现顺序硬约束**：`_append_behavior` 的 A2″ 累积改造**必须先于**条件甲的 `decision_rules` 改造落地；或两者同批落且以改造后语义为准。**不得先做甲。**

## 证据边界

- 三 handler 字段处置为本次**行级核实**（`merger.py:288-308 / 336-353 / 356-403 / 407-420 / 424-460`）。行号仍在漂移（第一单合入中），**以符号名定位**。
- 「`_merge_parameters` 可作 A2″ 累积范本」是我读其实现后的**判断**，**未与 Theory/Design 会签确认**该模式是否满足 A2″ 全部条款要求（如是否需要保序、是否需要记录累积来源）—— 请以专家裁定为准。
- 异议原文我**未直读**，依据为 leader 转述；v2.3 解决异议②一事由 leader 确认。
