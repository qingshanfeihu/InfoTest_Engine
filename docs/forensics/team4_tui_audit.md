...
- **裁决（leader 2026-07-17）：维持现状,标「已知行为」（by design 的账实分离代价），不做编写期 counts 实时化。** 理由三条:①P1-1 已修真正的用户困惑源（整个编写期卡「准备」）且实弹确认;②fork 卡片是编写期实时进度源,footer 数字 barrier 后校准可接受;③「视图短暂领先事实流」触 INV-7 核心原则（状态=事实流派生视图,缓存不得先于事实）,为一个有替代信息源的显示滞后开架构口子不值。**改进需 INV-7 架构评审**方可动——归 #15 低优先池,当前不改。
