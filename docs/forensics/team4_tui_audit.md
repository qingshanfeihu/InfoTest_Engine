...
## 14. 工具寻址：cmux surface 三态实测（2026-07-20，TUI-Eng 遇阻 + User-Obs 补全）

审计取证的前置工具事实，非产品缺陷，记此备后续抓屏免踩。

- **实测三态**（surface `1D30248F-976B-4BC2-B4BC-F910314AFD0D`，infotest PID 34041，HEAD=1e0e0298）：

  | 传参形式 | 结果 |
  |---|---|
  | UUID 全串 | ✅ 通 |
  | `surface:20`（ref 形式） | ✅ 通（User-Obs 补测） |
  | 裸数字 `20` | ❌ `Error: Surface index not found`（TUI-Eng 首次遇阻） |

- **要点**：`cmux --help` 声称 index 可用，**对 surface 实测不成立**——裸数字不解析。团队内转发 surface 时给 UUID 或 `surface:N` ref 形式，别给裸数字。
- **附带**：`cmux list-surfaces` 不是有效子命令（`Unknown command`）。

### 已销项：61 列窄 pane 折行（cosmetic，非渲染缺陷）

TUI-Eng 首次抓屏见工作目录路径在 `S|ynologyDrive` 处硬折行、输入框边框与提示文字挤同行，**当时未判定**（挂「需宽 pane 对照才能定性」）。User-Obs 以 146 列宽 pane 对照 → 布局正常，**定性=窄宽下的必然降级，非渲染缺陷，销项**。同型纪律：单宽度抓屏不足以判布局缺陷，需宽窄两面对照。

### 已销项：启动画面模型显示 `deepseek-v4-pro`（配置面有意状态，非渲染项）

TUI-Eng 首次抓屏见模型行显 `deepseek-v4-pro`，非 CLAUDE.md 所述主档 mimo，**当时按「配置面事实、非渲染错」如实上报未判定**。team-lead 核 `environment` 文件确认 `IST_MODEL=deepseek-v4-pro` / `IST_FLASH=deepseek-v4-flash` 是**有意配置状态**——**TUI 显示与配置一致 = 渲染正确，销项，非拦截项**。同型纪律：屏面值与文档描述不符时，先核配置源再判渲染层，别把配置漂移记成渲染缺陷。

> 至此 TUI-Eng 2026-07-20 首轮抓屏（PID 34041，HEAD=1e0e0298）两项待确认全部关闭，**启动画面渲染无发现**。卡片模式 / footer 九态聚合 / ask 面板交互本轮未触发，待 B 批实弹抓屏。

---

## 15. ink 全域 code review（2026-07-20，leader 派单：审 + 低风险自修）

范围 `main/ist_core/ink/`（8084 行）+ `tui/reducer.py`。四维：reducer 兜底 / ask 状态机 / 计数一致性 / 死码重复。
**测试终数 409 passed**（tests/ink + tests/tui + test_ask_user，随机序 ×4 稳定）。未 commit。

### 15.1 已修 6 项（低风险，行为语义零变化）

| # | 位置 | 问题 | 修法 |
|---|---|---|---|
| R1 | `ist_app.py::_handle_input` | 全链无 try/except（`ink/app.py` 只捕 OSError），渲染异常逃逸即**打死 ink-input 线程→整会话键盘失灵**。潜伏型：畸形 payload 活路径被 `reducer._notify` 吞、长驻 snapshot，**ctrl+o 重放才引爆** | 兜底 + 记日志；`except Exception` 不捕 SystemExit/KeyboardInterrupt |
| R2 | `ist_app.py` 编写期 bar | fork 卡跨轮累积（仅新 run/快照重放时清）且含 attributor 卡 → `fork_done>total` → `"░"*(barw-filled)` 得空串 → bar 25 字符撑破 footer 定宽、显 `42/34` | `done=min(fork_done,total)` + `max(0,...)`；实算：旧 25 长/`42/34`，新 20/`34/34` |
| R3 | `ist_app.py` 编写期计数段 | `other_seg` 只拼进非编写分支——而**编写相位正是引擎侧新增状态最可能出现处**，漏投状态在此静默消失，恰好绕过 2026-07-16 装的总量防线；欠定/失败同样不可见 | 非零才挂，保 F-TUI-8「不冗余显示全 0」 |
| R4 | `ist_app.py:_secs_int`（:511 调用点） | 裸 `int()` 对小数字符串 ValueError、对 dict TypeError，经 R1 那条链即致命 | 新 helper（float 中转+兜底） |
| R5 | `ist_app.py` 重复方法定义 | `append_transcript_info`/`set_background_status` 类内**各定义两次**，Python 静默丢弃前者。两份 append **已漂移**（死份 `f" {text}"` 单空格无 dim、活份双空格+dim）——照死份读代码得错误结论 | 删死份 + 活份 docstring 记录该实证；补 AST 机器门防复发 |
| R6 | 5 个孤儿模块 | `component.py`/`termio/sgr.py`/`termio/types.py`/`layout/geometry.py`/`termio/esc.py` 共 **405 行**，零 importer、`__init__.py` 全是纯 docstring 无再导出、零动态导入 | `git rm`；删后 ink 包导入自检通过 |

**一处否掉子审计建议**：它提议 :511 改用既有 `_fmt_secs` 去重，但 `_fmt_secs(223)` 输出 `3m 43s` 而非 `223s`，会改用户可见格式并破 `test_fork_cards_render.py:171` 的 `"223s/1440s"` 断言——**去重不能以改语义为代价**，改用保格式的新 helper。

### 15.2 只报不修（语义/跨域，待裁）

- **ASK-1（P1，探针实证）**：末题「还有N题未答」挡板 armed 后再按**数字**，用**旧选择**直接提交、静默丢弃本次数字意图 → 记下的是**错答**（选 b2 告警后按 "1"，仍交付 `b2`）。根源 `ask_user_view.py:188` 为解死锁开的放行口，**死锁经验证是真的**（不放行则数字永远重新 arm）。修法需重设计。
- **FOOTER-1（P1，跨域→Py-Eng）——✅ 已关闭（2026-07-20 当日，Py-Eng 落地）**：原问题＝`_shared.py` 把 `suspended` 折进 `failed_terminal` → 渲染成**失败N**，但 `views.py:33` 明写 suspended 是**非终态**（下批同参续跑，§14-R4/§11.9 走 `unfinished/`）；同类 `broken_blocked`（床不可达）折进 `failed_active` → **死床显示成满批在编写**。
  修法（Py-Eng）：`suspended`→`pending_decision`（欠定）、`broken_blocked`→独立 `broken` 桶，**TUI 零改动**——`broken` 是 ist_app 未具名消费的新键，经减法落既有「其他N」残差桶。
  **组合实测**（TUI-Eng 复核，两处修复叠加）：全批 `broken_blocked` 死床 → 桶投影 `{'broken': 34}` → 渲染 `产出0 编写中0 欠定0 通过0 失败0 其他34`。①死床不再伪装成「编写中」；②因 `broken` 不属 `spin`，`_authoring` 判据不再成立，也不会误挂「产出将在合并时结算」。**残差桶这道 2026-07-16 装的防线在真实新状态上生效了一次**——它接住了一个前端完全没预期的新键。
  R3（编写期补显欠定/失败/残差）仍有独立价值：spin>0 或 fork 在跑**且**同时存在欠定/失败/残差时，编写期才不会把它们藏掉。
  遗留（P2 池，我侧一行）：`broken` 目前显为通用「其他N」，若要专属「异常N」标签需在 `_render_engine_bottom_line` 具名消费该键。
- **ASK-2（P2）**：两题题干文本相同 → answers dict 键冲突，前题答案整条消失（实证）。该键是与上游 `ask_user` 工具共享的接口契约（tool `__init__.py:265` 同样按题干建键），非渲染层能单独改。
- **CARD-1（P2）**：卡片计数与 footer 计数**无共同数据源**（卡走 fork 生命周期事件、footer 走 facts 视图），卡集跨轮不清、不滤 skill。R2 只做显示夹紧止血，**跨轮累积本身待裁**（按轮清 vs 只计 worker skill）。
- **待评估池（子审计提出，我未逐条复核，动手前需再验）**：行数估算两套算法分歧（`layout/engine.py` 不剥 ANSI、无缩进感知续行宽 vs `string_width.py` 自称单一事实源）＝唯一有活跃正确性后果的重复；5 份 ANSI 正则；`render.py` 背景色表缺亮色项致 `background_color="gray"` 被静默丢弃；`screen.py` `len(code)==5` 脆弱背景判定使组合码 `\x1b[1;41m` 选区高亮失效；ask 已答摘要实时/重放两份手工克隆已漂移。

### 15.3 方法论记录（两条，值得复用）

1. **`git grep -E` 不支持 `\b`**（POSIX ERE）。我首轮用 `\b` 查孤儿模块引用得「五个全零命中」，差点据此删文件——实为**查法失效的假阴性**。用已知必然命中的串（`NamedColor`）反证查法后才重查。同型：`[[signal-licenses-only-what-measured]]`「查无先证伪自己查法」。
2. **多人共享工作树下，全量 pytest 的失败要先归属再定性**。本轮全量出现 4 failed + 间歇收集 error，逐一溯源后**全部落在其他成员在改的文件**（`_shared.py`/`test_footer_projection.py`/`verifiability_tool.py`/`batch_tools.py` 均为他人 `M`，`test_needs_decision_ledger_safety.py` 为他人未跟踪新增）；间歇 error 的串溯到 `batch_tools.py:1182` docstring＝撞上他人中途保存。**HEAD worktree 不能当基线**——干净检出缺 untracked/gitignored 资产（`environment`、runtime），会假报 64 failed。可靠做法＝按文件归属 + 自己域随机序多次复跑。
