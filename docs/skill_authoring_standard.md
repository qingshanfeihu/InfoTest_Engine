...
when_to_use: |                                # 必填，触发条件 + SKIP 条件（四行格式，各占一行）
  Use when ...
  Examples: "...", "..."          # 用户会说的真实消息样例，越贴近真实越好
  Trigger keywords: ...           # 逗号分隔的用户高频用词；必须独占一行、行首为 "Trigger keywords:"
  SKIP when: ...
...
**`Trigger keywords:` 必须独占一行、行首即该字面串**（中英冒号皆可）。listing 的触发词提取靠按行
匹配该前缀；若把 keywords 揉进段落中间或换成别的措辞（如 `Trigger phrases`），虽兼容但不推荐——
统一用 `Trigger keywords:`。`SKIP when` 子句尤其重要，否则通用 QA 会误调。

> skill listing 由 `middleware/per_turn_skill_reminder.py` **单路**注入（每轮一条 system-reminder
> HumanMessage，含 BLOCKING REQUIREMENT 措辞）。deepagents 原生 SkillsMiddleware **已卸载**（它教模型
> 用被屏蔽的 read_file 读 skill 全文，是死指令且与本路冲突）。详见 `skill_progressive_disclosure_fix.md`。
...
try:
    from main.ist_core.agents.<your_subagent_module> import build_<subagent_name>
...
```python
class IstCoreState(TypedDict, total=False):
    # 兼容别名：QaAgentState = IstCoreState（见 main/ist_core/__init__.py）
...
1. ✅ 在 `main/ist_core/skills/<skill-name>/` 创建目录
...
6. ✅ 测试：`tests/ist_core/skills/test_<skill_name>_*.py`
7. ✅ 跑 e2e：`runner.py "<触发词>..." --stream`
8. ✅ 跑通用 QA："如何配置 X"（不应误触新 skill）

## 十一、参考实现

- 评审类完整实现：`main/ist_core/skills/test-case-review/`
- 主 agent prompt：`main/ist_core/agents/_prompt.py`
- Skill 注入中间件：`main/ist_core/middleware/per_turn_skill_reminder.py`
- 硬闸节点：`main/ist_core/nodes/review_gate.py`
- 工程兜底：`main/ist_core/graph.py:finalize`
