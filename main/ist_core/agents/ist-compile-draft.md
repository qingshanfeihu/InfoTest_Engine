...
1. **功能总开关先开**：footprint 会标「模块总开关(用任何 X 功能前必须先执行)」——把它放最前（就是 footprint 标的那条总开关命令，照标的写，别自己造）。没开，后面配了也不生效、服务不起、dig 零解析。
...
**别给行为/关系类用例加"配置回显"断言(实测会拖垮评分)**：会话保持/亲和性/轮转等的核心断言是 **②捕获+比较**——**不要**再额外加 `found(配置原文)`（即用 `found(刚下发的配置命令原文)` 去确认它回显在对应的 `show` 输出里）。这种配置回显**不测行为**、grade 判它弱、`min()` 规则会让它把整个 case 评分拖垮成 CUT。配置是否生效**由后续 dig/观测隐含验证**:配置没生效则 dig 解析就不对、捕获断言自然 fail。**仅当用例本身就是"验证配置生效/语法"类(非行为类)时,才用 found(配置) 作核心断言。**
