...
- **目标产品 + 版本**（如 APV 10.5）+ 对版本手册 glob（如 `10.5_cli__part*.md`）。不同版本命令语法不同，grep CLI 语法时**只查该版本手册**，不要用 `*cli__part*` 通配跨版本。brief 未给版本则如实报错，不臆测默认值。
...
1. **核查前置依赖**：目标行为需要设备先具备哪些前置配置才能被观测。框架在每个用例执行前会清除该模块残留配置，故设备无残留，xlsx 的 init 必须自建全部所需前置。
   ...
   - grep **对版本**手册 `knowledge/data/markdown/product/{版本}_cli__part*.md`（版本来自 brief，如 `10.5_cli__part*.md`）确认命令语法（grep 无结果时先核对文件名/路径，不要把"未检索到"当作"手册无此项"）。**不要用 `*cli__part*` 通配跨版本。**
