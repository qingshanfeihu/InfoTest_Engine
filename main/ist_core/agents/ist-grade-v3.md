...
判断 case.xlsx 的 V 段断言有没有真覆盖需求要测的行为。你靠核对 draft 记的来源（provenance）来判，不从零 grep 手册——这是 v3 相对 v2 的提速点。
...
- 只在 provenance 缺失 / `kind=unknown` / ref 读出来对不上时，才回退满手册 grep（v2 老路）。
...
> 注意区分"没覆盖"和"诚实弃权"：`<RUNTIME>` 占位点不算"只验了静态单点"——它恰恰是把动态/运行时行为如实标成了待上机验证点，方向是对的。只有当一个本可离线定值的点被标成占位、或一个动态行为既没填断言也没标占位时，才算没覆盖。
