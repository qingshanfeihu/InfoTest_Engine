...
① 欠定通道(needs_decision.json 台账 → ask_decision 面板):台账是锚——问题文本由
   代码从台账拼装,autoid 全名、三选项、ordering_sensitive 的「顺序语义保留/放弃」
   显式句都由代码保证;`IST_ENGINE_ASKQ_LLM=1` 时允许 LLM 润色散文,润色后仍过
   `validate_questions`(不过则回落模板)。每次 ask ≤4 题(ask_user 工具硬限),超出分批。
...
② 矛盾/止损通道(ask_contradiction interrupt → 面板):panel/cap/env/bed/contra/
   ...
   2026-07-16 P0 收编自 engine_tool._contradiction_question 与 nodes._answer_token
   (517027 实弹三重缺陷的修法:缺陷臂/claim 历史/意图归类在本文件;止损记账如实化
   由甲队以 user_stop 事件分离形态落在 nodes;接线与分工见 docs/forensics/
   team3_impl_ask.md)。
...
# 2026-07-16 P0-新②/N2 收编:题面组装与 Other 意图归类从 engine_tool/nodes 迁入
# 本文件(ask 面板语义的单一事实源)。接线:engine_tool._contradiction_question →
# 委托 build_ask_question;nodes._answer_token → 别名 answer_token;nodes stop/
# downgrade 落账分支 → 消费 stop_accounting(甲队文件,接线包见交付文档)。
...
# 止损落账(P0-新②b,键名 user_stop 冻结)已由甲队按 N1a 事件分离形态落地在
# nodes.ask_contradiction:独立 `ev:"user_stop"` 事实承载真相 + r99 attribution 保留
# env_blocked 但带 `user_stop:true` 标记声明「记账行,非环境结论」(路由零变化、旧
# facts 天然兼容;render 取语义归因时跳过记账行)。语义单一事实源在彼处
# (tests/ist_core/compile_engine_v8/test_claim_stickiness.py 锁),本文件不再输出
# 落账形态——题面侧的对应承诺是「停止该案」选项文案:记为你的停止裁决,不覆盖
# 在案技术判断(与 render 跳记账行的行为成对)。
...
def _defect_intent(a: str) -> bool:
    """自由输入的缺陷意图(带否定门:「不是缺陷,继续修」不算)。517027 实弹:
    用户在 cap 面板写「是缺陷」曾被恒归并成 continue(授权再修 2 轮)——最恶性误译。"""
    s = _NEG_DEFECT_RE.sub("", str(a or ""))
    low = s.lower()
    return any(w in low for w in ("缺陷", "产品问题", "提单", "bug"))
