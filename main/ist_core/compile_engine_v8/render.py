...
DISP_CN = {"reflow": "带反馈重新编写", "frozen": "原方法已证无效,换法重编",
           ...
           # M-18:缺词条会 fallback 英文 token 上报告
           "expectation_suspect": "预期值可疑(需核对出处)",
           "transient": "偶发波动(重跑消失)"}
    ...
    for e in entries:
        ...
        if trail:
            ...
            for t in trail:
                ...
                # M-17:round==99 含引擎自动终局(deesc_auto,user_confirmed=False)——
                # 不得一律渲染成「用户裁决」(与同文件 user_confirmed 自相矛盾)。
                if t.get("by_user"):
                    r = "用户裁决"
                elif int(t.get("round") or 0) == 99:
                    r = "引擎自动裁决"
                else:
                    r = f"第{t.get('round')}轮"
