...
class PlanPanel:
    ...
    def update(self, todos: list[dict[str, str]]) -> None:
        ...
        bottom_gap = create_element(NodeType.BOX)
        bottom_gap.style.height = 1
        bottom_gap.append_child(create_text(""))
        self._node.append_child(bottom_gap)
        self._node.style.height = len(self._todos) + 3

    def clear(self) -> None:
        self.update([])

    def mark_all_done(self) -> None:
        """Run 结束时把所有未完成 task 标记为 completed 并重渲染。"""
        if not self._todos:
            return
        done_todos = [
            {**t, "status": "completed"} for t in self._todos
        ]
        self.update(done_todos)
