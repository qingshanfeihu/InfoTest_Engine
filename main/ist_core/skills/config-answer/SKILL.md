...
**搜索优先级**：
1. **首选** grep `knowledge/data/markdown/product/app_*.code_format.json`，查找该业务的**完整配置示例**。app 文档来自产品配置指南，含可直接使用的示例（创建 virtual server + real server + group + health check 的完整序列）。找到示例后，修改其中的 IP/端口/名称即可直接使用，无需逐命令拼装
2. 主力 grep `knowledge/data/markdown/product/cli_*_commands.md`（纯文本，命中精准）
3. 补充 `cli_*part*.code_format.json`（Mineru 导出 JSON，CLI 在 `markdown` 字段）
