...
只给 `slb group method` 不给 real server/virtual server/policy = 不完整。必须从底层资源开始逐层创建。
...
4. 所有引用资源是否已在前面命令中创建？
5. 需要手动启用的资源已追加启用命令？
6. **服务栈完整？**（SLB：real server + group + virtual server + policy 都有？SDNS：on + host + service + pool + listener 都有？）
