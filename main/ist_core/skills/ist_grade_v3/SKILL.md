---
name: ist_grade_v3
description: v3 slimmed grade subflow — judges ONLY whether V-segment assertions cover the requirement's target behavior, and VERIFIES the draft's Provenance IR (case.provenance.json) instead of re-grepping the manual from scratch. For each V-layer step it confirms the cited source actually supports the expected value; only falls back to grep when provenance is missing or suspicious. Structural validity stays the emit gate's job. Read-only. Invoked by ist_compile_v3; takes a structured brief as $ARGUMENTS.
context: fork
agent: ist-grade-v3
...
- **不要因为它没填具体值就 CUT**。它的真实值由 `ist_verify_v3` 上机回填锁死，编译期本就不该有值。
