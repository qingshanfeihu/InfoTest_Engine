...
def submit_answers(question_id: str, answers: dict[str, str]) -> bool:
    ...
    if evt is not None:
        evt.set()
    # 答后残留修复:答复只 set Event 不回 reducer,全量重放会复活已答面板——此事件让 reducer 按
    # question_id 标块 answered、重放渲折叠态(见 DESIGN_dongkl_finalization.md §2;根因链 docs/forensics/F_closeout.md TODO4)。
    try:
