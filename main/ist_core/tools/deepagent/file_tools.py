...
from main.ist_core.tools.deepagent._range_reader import read_text_range
from main.ist_core.tools.deepagent._rg import RipgrepResult, run_ripgrep
...
# macOS / Windows 文件系统大小写不敏感：'MAIN'、'Memory'、'Runtime' 等大小写变体
# 在底层解析到与黑名单同一目录，但区分大小写的集合成员检查会漏判。
# 统一用小写副本做成员判断，闸口两侧都 .lower()。
_PLATFORM_DENIED_TOP_LEVEL_LC = {x.lower() for x in _PLATFORM_DENIED_TOP_LEVEL}
_PLATFORM_DENIED_FILES_LC = {x.lower() for x in _PLATFORM_DENIED_FILES}
...
def _resolve_inside_root(raw_path: str | None, *, must_exist: bool = False) -> Path:
    ...
    if rel_to_project is not None:
        ...
        if rel_parts:
            top = rel_parts[0]
            if top.lower() in _PLATFORM_DENIED_TOP_LEVEL_LC:
                raise PermissionError(
                    f"path is in platform-denied directory: {top}/"
                )
            if len(rel_parts) == 1 and top.lower() in _PLATFORM_DENIED_FILES_LC:
                raise PermissionError(
