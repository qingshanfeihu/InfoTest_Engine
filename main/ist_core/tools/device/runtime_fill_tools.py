...
配合 ist-fill fork：fork 先 qa_run_batch/qa_run_case 拿设备真实输出，再 qa_list_runtime_slots
看有哪些待填槽位（及各自的前序观测命令），把每个槽位的真实值从设备输出里抽出来，
调 qa_fill_runtime 锁死写入。抽不出就给空值＝如实留空，绝不猜。
...
def _resolve(xlsx_path: str):
    """走 agent 沙箱多根解析定位 xlsx（与 qa_run_case 一致）。"""
    p = None
    try:
...
def qa_list_runtime_slots(xlsx_path: str) -> str:
    ...
    用法：上机跑完（qa_run_batch / qa_run_case）拿到设备逐步骤真实输出后，调本工具看有哪些槽位，
    把每个槽位对应的真实值从设备输出里抽出来，再调 qa_fill_runtime 写入。
...
@tool(parse_docstring=True)
def qa_fill_runtime(xlsx_path: str, fills_json: str, run_meta: str = "") -> str:
    ...
    **永不覆盖已填值**（锁是结构性的，不靠你自律）。所以同一槽位你只能填一次，填错也不会被你
    ...
    你给的值必须来自**设备真实输出**（qa_run_batch / qa_run_case / qa_probe_show 拿到的）。
    抽不出某槽位的真实值就给空值（或干脆不传该槽位）＝如实留空，**绝不猜一个**。

    fills_json 是 JSON 数组，每项三个键：slot_id（来自 qa_list_runtime_slots）、
    ...
    lines = [f"=== qa_fill_runtime ===", res.summary(),
