...
def _build_chat_model(provider: str, model_name: str, **kwargs: Any):
    ...
    if provider == "deepseek":
        ...
        # DeepSeek v4-pro thinking 模式实测降低评审质量（18 条 thinking=disabled
        # vs 15 条 thinking=enabled），且 reasoning_effort=max 自动升档导致单次
        # invoke 186s。直接关 thinking，让模型全部 token 预算用于 content。
        ...
        # Qwen3.6 在 tool-calling 模式下默认不输出陪伴 content；开启 thinking 后
        # 模型把推理过程放进 message.reasoning_content / 流式 delta.reasoning_content，
        # TUI 渲染成"思考"通道。参考百炼 OpenAI 兼容端点文档（extra_body.enable_thinking）。
...
#: 每 tier 对应不同复杂度任务：opus = 最强（评审 / 推理），sonnet = 中等
#: （QA / 文档问答），haiku = 最弱（路由 / 补全）
TIER_ENV_VARS = {
    "opus": "IST_OPUS_MODEL",
