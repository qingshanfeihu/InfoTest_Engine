...
class PromptInput:
    ...
    def __init__(
        ...
        *,
        ...
        self._value = ""
        self._cursor_pos = 0
        # Pasted-text store: id → original text. The visible prompt shows
        # short [Pasted text #id +K lines] placeholders; expand_pasted_refs()
        # swaps them back before submit.
    ...
    def clear(self) -> None:
        self.set_value("")
        # Drop the paste store too — Esc / clear() means "throw away the
        # current draft", and a stale paste id would otherwise live on
        # in _pasted_contents indefinitely.
    ...
    def handle_paste(self, text: str) -> None:
        ...
        """
        # Normalize CR/CRLF to LF for line counting + storage.
        ...
        if is_long:
            ...
            return
        # Short paste — insert verbatim with visible newline markers.
    ...
    def clear_pasted_refs(self) -> None:
        ...
        self._pasted_contents.clear()
        # Keep _next_paste_id monotonic so old refs left in history can't
