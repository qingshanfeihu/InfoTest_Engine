...
class Output:
    ...
    def _apply_write(self, op: WriteOp) -> None:
        ...
        import re
        # Split text into segments: plain text and ANSI SGR sequences
        # Pattern matches \x1b[...m sequences
        ...
        for segment in segments:
            if not segment:
                continue
            # Check if this is an ANSI SGR sequence
            if segment.startswith("\x1b[") and segment.endswith("m"):
                # Parse SGR and create/lookup style
                if segment == "\x1b[0m":
                    current_style = op.style_id  # reset to base style
                else:
                    # Combine base style codes with new codes
                    base_codes = self._style_pool.get(op.style_id)
                    current_style = self._style_pool.intern(base_codes + [segment])
                continue

            # Plain text — render character by character
            lines = segment.split("\n")
            ...
            if row_y >= self._height:
                break
        # No implicit row_y += 1 at end — caller handles layout
