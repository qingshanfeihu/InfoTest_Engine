...
def start_selection(s: SelectionState, col: int, row: int, *, alt: bool = False) -> None:
    ...
    # Focus is not set until the first drag motion. A click-release with
    # no drag leaves focus None → has_selection / selection_bounds return
    # False/None via the `not s.focus` check, so a bare click never
    # highlights a cell.
    s.focus = None
    s.is_dragging = True
    s.anchor_span = None
...
def update_selection(s: SelectionState, col: int, row: int) -> None:
    if not s.is_dragging:
        return
    # First motion at the same cell as anchor is a no-op. Terminals in
    # mode 1002 can fire a drag event at the anchor cell (sub-pixel
    # tremor, or a motion-release pair). Setting focus here would turn
    # a bare click into a 1-cell selection and clobber the clipboard
    # via release-copy. Once focus is set (real drag), we track normally
    # including back to anchor.
    if (
        s.focus is None
...
def finish_selection(s: SelectionState) -> None:
    ...
    # Clear via clear_selection() on Esc or after copy.
...
# Unicode-aware word character matcher: letters (any script), digits, and
# the punctuation set iTerm2 treats as word-part by default — matches the
# muscle memory most macOS terminal users have.
...
def _word_bounds_at(
    ...
    # If the click landed on the spacer tail of a wide char, step back
    # to the head so the class check sees the actual grapheme.
    c = col
    if c > 0:
    ...
    while hi < width - 1:
        ...
        if nc.width == CELL_SPACER:
            # Include the spacer tail in the selection range (it
            # belongs to the wide char at hi) and continue past it.
            hi = nxt
            continue
...
# ---------------------------------------------------------------------------
# Word / line selection (multi-click)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Bounds, hit-testing, text extraction
# ---------------------------------------------------------------------------
...
def _extract_row_text(
    ...
    if content_end > 0:
        return line
    # Trim trailing whitespace on the last fragment of a logical line.
    return line.rstrip()
...
def capture_scrolled_rows(
    ...
    if side == "above":
        ...
        # Reset anchor.col so the next tick / final get_selected_text
        # reads the full row at this anchor row.
        if (
        ...
        # Below: newest at FRONT, closest to on-screen content
...
# ---------------------------------------------------------------------------
# Keyboard scroll / drag scroll shifts
# ---------------------------------------------------------------------------
...
def shift_selection(
    ...
    # Truncate over-debt accumulator (stale entries from cleared virtual)
