...
用法::
    ...
    # 单次查询
    python -m main.qa_agent.runner "如何配置 HTTP/2 SLB？"

    # JSON 文件输入（结构化）
    python -m main.qa_agent.runner --input sample.json
...
def run_single(
    ...
    *,
    ...
    if stream:
        # M4：接事件总线 + Sink；这里先提供最小化实现（直接打印 chunk content）
...
def main() -> None:
    ...
    # Windows PowerShell `>` 默认 UTF-16 LE BOM 重定向；强制 stdout/stderr UTF-8
    try:
