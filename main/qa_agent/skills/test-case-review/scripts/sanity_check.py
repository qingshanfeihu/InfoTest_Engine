...
_MAX_LOCATIONS = 20  # 每类异常最多列 20 条位置（仿 xlsx recalc.py）
...
def _read_table_rows(content):
    ...
    header = None
    rows = []
    current_block = ""  # 当前 Item|Sub Item 块标题（沿用上一非空值）
    block_cols = ()
    ...
    for i, line in enumerate(lines, 1):
        ...
        # 沿用块标题（Item / Sub Item 单元格为空时沿用上一非空）
...
def check_outlier_identifiers(content):
    ...
    for label, pattern in [
        ...
        if len(most) < 2:
            continue
        top = most[0]
        # 离群定义：出现次数 ≤ 3 且 ≤ top/20 次（比如 vs11=81 次时，vs12=3 次明显离群）
        # 但 g1/g2/g3 这种"不同 group 名"不算离群——只有 top 远超其他时才有意义
        threshold = max(3, top[1] // 20)
        outliers = [(name, cnt) for name, cnt in most if cnt <= threshold and cnt < top[1] // 5]
        if outliers and top[1] >= 20:  # top 至少出现 20 次才认为有"离群"概念
            issues[label] = {
...
def check_type_marking_consistency(rows):
    ...
    """
    issues = []
    # 抽 description 末尾的"<cmd> ?"模式（CLI help 测试）
    help_pattern = re.compile(r"^[\w\s]+\?$")
    help_groups = {}  # type → [(line, desc)]
    for r in rows:
        d = r.get("Description", "").strip()
...
def check_section_redundancy(rows):
    ...
    sub_names = [s for s in groups if len(groups[s]) >= 5]  # 至少 5 条用例的子块才比
    redundant_pairs = []
