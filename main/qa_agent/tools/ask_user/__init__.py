"""qa_ask_user: 仿 Claude Code AskUserQuestion 工具。

CC 二进制反编译要点（来自 ~/.npm-global/lib/node_modules/@anthropic-ai/claude-code/bin/claude.exe）：
...
- 响应回喂：cc 用 tool_result 文本格式 `User has answered your questions: "Q"="A"`，
  不用 system-reminder 标签
...
# 全局 pending question 注册表 — 供 TUI bridge 拿到当前 pending question + 回写答案
...
def qa_ask_user(questions: list[dict[str, Any]]) -> str:
    ...
    # 通过 EventBus 广播给 TUI（同 main_agent thinking 通路）
    try:
