"""IST-Core 工具沙箱：CWD 解析层（仿 cc-haha cwd.ts + filesystem.ts）。
...
cc-haha 设计哲学（核实 ``cc-haha/src/utils/permissions/filesystem.ts:667-674``
+ ``utils/cwd.ts:1-33`` + ``Tool.ts:783-792``）：
...
- ``_default_cwd()`` ↔ cc-haha ``pwd()``——当前简化为读 ``_agent_roots()[0]``
- ``_resolve_cwd_for_target()``——根据目标路径选最匹配的沙箱根

**不做**：

- SandboxedToolBase 抽象基类（cc-haha 反对，用工具自实现 + 共享模块）
- contextvars cwd 隔离（无并发场景；``_default_cwd()`` 留好钩子）
- OS 级沙箱运行时（cc-haha sandbox-runtime 基于 bwrap，不可移植 Python）
...
def _default_cwd() -> Path:
    ...
    cc-haha 对照：``cwd.ts:1-33`` ``pwd()`` 读 AsyncLocalStorage store。
    InfoTest_Engine 简化为静态读首根，未来可升级 contextvars 驱动支持
    并发 agent 各自 cwd（钩子已留好）。
    """
    # 延迟 import 避免循环依赖（file_tools.py import 本模块的函数）
    from main.qa_agent.tools.deepagent.file_tools import _agent_roots
...
def _resolve_cwd_for_target(target: Path | str | None) -> Path:
    ...
    cc-haha 对照：``BashTool/pathValidation.ts:657-694`` 用 ``resolve(cwd, path)``
