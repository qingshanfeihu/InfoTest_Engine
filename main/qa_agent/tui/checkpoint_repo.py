...
class ThreadEntry:
    ...
    thread_id: str
    last_step: int
    preview: str  # 用户输入或最后一条 LLM 输出的前 60 字符
    timestamp: str  # checkpoint metadata.ts 或 ISO 时间
...
class CheckpointRepo:
    ...
    def _build_default() -> Any:
        ...
        try:
            from main.qa_agent.graph import _make_checkpointer  # type: ignore[attr-defined]
    ...
    def get_thread(self, thread_id: str) -> Optional[dict[str, Any]]:
        ...
        if tup is None:
            return None
        # CheckpointTuple.checkpoint 是 dict，state 在 channel_values 里
        try:
    ...
    def _tuple_to_entry(t: Any, tid: str) -> ThreadEntry:
        ...
        if isinstance(metadata, dict):
            ...
            step = 0
            preview = ""
        # checkpoint 时间戳：CheckpointTuple.checkpoint['ts']
        ts = ""
        try:
    ...
    def _extract_preview(writes: Any) -> str:
        ...
        for source_node, payload in writes.items():
            if isinstance(payload, dict):
                # 常见字段：user_input / final_answer / messages
