...
四类语义（仿 cc-haha）：
  ...
  feedback             反复纠正的反馈
  project              项目动态 / 决策 / 状态
  reference            外部引用（Linear / Slack / 文档链接）"""
...
def _cmd_overview(app: "IstApp") -> SlashCommandResult:
    ...
    if long_dir.exists():
        ...
        for top in ("preferences.md",):
            ...
            if f.exists():
                long_files.append(f)
        # 子目录类（feedback / project / reference / review_conclusions）
...
def _cmd_status() -> SlashCommandResult:
    try:
        from main.qa_agent.memory.dream import (
            should_run_dream,
            _last_run_path,
        ...
        # 注意：should_run_dream 副作用是会拿 PID 锁——立即释放
        try:
            from main.qa_agent.memory.dream import _release_pid_lock
            _release_pid_lock()
        except Exception:
            pass
