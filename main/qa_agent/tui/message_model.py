...
# 空只读 Mapping —— 避免 ContentBlock 默认 input/payload 用可变 dict（frozen 不允许）
_EMPTY_MAP: Mapping[str, Any] = MappingProxyType({})

# ---------------------------------------------------------------------------
# Block types —— BetaContentBlock + IST-Core 特有事件
# ---------------------------------------------------------------------------

#: 文本 / LLM 输出
BLOCK_TEXT = "text"
#: 模型 thinking 块（DashScope reasoning_content / Anthropic thinking）
BLOCK_THINKING = "thinking"
#: 工具调用（assistant 一侧）
BLOCK_TOOL_USE = "tool_use"
#: 工具结果（user 一侧，配 tool_use_id）
...
#: write_todos 工具的快照（plan_panel 派生用）
BLOCK_TODO_LIST = "todo_list"
#: HIL interrupt / decision
...
class ContentBlock:
    ...
    output: str = ""
    is_error: bool = False
    # 通用 payload —— phase_marker / evidence / finding / todo_list / hil_*
...
# ---------------------------------------------------------------------------
# 工厂函数 —— createAssistantMessage / createUserMessage
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 不可变更新 helper —— 替换 tuple 内某个 ContentBlock
# ---------------------------------------------------------------------------
