...
def test_fingerprint_stable_and_args_sensitive():
    a = _tool_call_fingerprint("fs_grep", {"pattern": "slb real tcp", "path": "x"})
    b = _tool_call_fingerprint("fs_grep", {"path": "x", "pattern": "slb real tcp"})
    c = _tool_call_fingerprint("fs_grep", {"pattern": "slb real udp", "path": "x"})
    assert a == b  # key 顺序无关
...
def test_dup_count_detected():
    ...
    for i in range(3):
        tc_id = f"call_{i}"
        msgs.append(_ai_with_tool_call(tc_id, "fs_grep", {"pattern": "slb real tcp ", "path": "p"}))
...
def test_alternating_loop_detected():
    ...
    for i, pat in enumerate(pats):
        tc_id = f"call_{i}"
        msgs.append(_ai_with_tool_call(tc_id, "fs_grep", {"pattern": pat, "path": "p"}))
        msgs.append(_tool_result(tc_id, "(no matches)"))
    stats = _analyze(msgs, window=8)
...
def test_reminder_injected_on_dup(monkeypatch):
    ...
    for i in range(3):
        tc_id = f"c{i}"
        msgs.append(_ai_with_tool_call(tc_id, "fs_grep", {"pattern": "bindgroup", "path": "p"}))
        msgs.append(_tool_result(tc_id, "(no matches)"))
...
def test_no_reminder_when_healthy():
    ...
    mw = LoopGuardMiddleware()
    msgs = [HumanMessage(content="翻译配置")]
    msgs.append(_ai_with_tool_call("c0", "fs_grep", {"pattern": "slb virtual http", "path": "p"}))
    msgs.append(_tool_result("c0", "cli.md:7212: slb virtual http knownsvc"))
    msgs.append(_ai_with_tool_call("c1", "fs_grep", {"pattern": "slb group", "path": "p"}))
...
def test_soft_budget_reminder_is_gentle_not_stop(monkeypatch):
    ...
    for i in range(6):
        ...
        msgs.append(_tool_result(tc_id, f"produced structurally-correct workspace/outputs/case{i}/case.xlsx"))
    out = mw._maybe_reminder_messages(_Req(msgs))
