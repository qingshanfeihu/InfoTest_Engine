...
def test_trace_fork_writes_line(tmp_path, monkeypatch):
    ...
    assert "fork=compile-worker" in content
...
def test_trace_fork_never_raises(tmp_path, monkeypatch):
    ...
    importlib.reload(L)
    # 不抛异常即通过
    L._trace_fork("compile-attributor", "brief", 1.0, {"ai_rounds": 1, "tool_results": 0, "tool_calls": {}})


def test_execute_fork_skill_populates_summary_sink(monkeypatch):
    """execute_fork_skill 经 summary_sink 把 fork 的工具调用/轮数回传给调用方
    （引擎据此聚合 LLM/查找成本，验证预检索是否真减少调用）。
    mock 掉真实 subagent 构建 + LLM 流式调用，不需 LLM/设备。"""
    import main.ist_core.skills.loader as L

    monkeypatch.setattr(L, "get_subagent_runnable", lambda name, **kw: object())
    canned = {"messages": [
        HumanMessage(content="task"),
        AIMessage(content="", tool_calls=[
    ...
    sink: dict = {}
    out = L.execute_fork_skill("compile-worker", "some brief", summary_sink=sink)
...
def test_execute_fork_skill_summary_sink_cleared_on_error(monkeypatch):
    ...
    out = L.execute_fork_skill("compile-worker", "brief", summary_sink=sink)
...
def test_execute_fork_skill_marks_recursion_limit(monkeypatch):
    ...
    out = L.execute_fork_skill("compile-worker", "brief")
    assert out.startswith("ERROR:")
