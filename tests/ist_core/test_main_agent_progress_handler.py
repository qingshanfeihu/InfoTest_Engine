...
def test_on_tool_error_skips_second_duplicate_callback() -> None:
    ...
    h.on_tool_start({"name": "qa_deepagent_grep"}, "query", run_id=run_id)
    h.on_tool_start({"name": "qa_deepagent_grep"}, "query", run_id=run_id)
    assert h._tool_name_stack == ["qa_deepagent_grep"]
...
def test_on_tool_error_after_dedup_start_only_pops_once() -> None:
    ...
    h.on_tool_start({"name": "qa_deepagent_read_file"}, "path", run_id=run_id)
    h.on_tool_start({"name": "qa_deepagent_read_file"}, "path", run_id=run_id)
