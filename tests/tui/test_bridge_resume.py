...
不实跑 graph——用 mock graph_factory 截获 invoke / stream_and_collect 入参。
"""
...
def test_bridge_resume_with_uses_langgraph_command_class():
    ...
    def fake_stream_and_collect(graph, state, *, config=None, sinks=()):
    ...
    with patch("main.qa_agent.tui.bridge.stream_and_collect", side_effect=fake_stream_and_collect):
        bridge.resume_with(decision)
        # 等后台线程跑完
        if bridge._worker is not None:
            bridge._worker.join(timeout=2.0)

    # 至少一次 stream_and_collect 调用
    assert invoked_args, "stream_and_collect should have been called by resume_with"
    call = invoked_args[0]
...
def test_bridge_resume_with_emits_run_done_event():
    ...
    with patch("main.qa_agent.tui.bridge.stream_and_collect", side_effect=fake_stream):
