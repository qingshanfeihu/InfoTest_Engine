...
def test_finalize_node_end_appends_ai_final_message(sink, captured):
    """评审等场景：graph.finalize 才合并 verifier 全文；必须用 finalize node_end."""
    sink(_ev(
        "node_end",
        payload={"final_answer": "VERDICT: PASS\nLEVEL: P2\nfull report…"},
        tags={"node": "finalize"},
    ))
    finals = [
        e for e in captured
        if e.kind == "append" and isinstance(e.message, AIFinalMessage)
    ]
    assert finals and finals[0].message.content.startswith("VERDICT:")
