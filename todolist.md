...
- 检索：`MemoryInjectionMiddleware` 自动注入（自然语言查询）+ `kb_footprint` tool（agent 主动按 CLI 命令名精确查）
...
**背景**：测试评审 skill 跑出 "AI 偷懒"——主 agent 复用 memory 历史结论、verifier subagent 没注册、qa_sanity_check 工具被错误调用、最终输出只有 15 字"评审完成"。整个改造对照业界 agent 框架的 verification 架构 + agent tool / skill tool / 通用检索代理设计。
...
- 主 agent system prompt 加多个反偷懒 sections：
  ...
  - `Communication Style`（通用 Tone and Style 约束）
  - Tool usage 加 `Parallel tool calls` + `run_shell 多命令`
- explore subagent 采用通用检索代理模式：加 "the caller will relay this to the user, so it only needs the essentials" + SHARED_PREFIX
- skill listing 加 `when_to_use` 字段（含 SKIP 条件）—— LLM 看到判定 skill 是否真匹配，避免通用 QA 误调
- 删除 `ask_user` 工具：评审场景被错误调用（options 必须 2-4 项的参数 derail）+ 通用 QA 不需要
...
- 新增 `review_gate` 节点（仿 `hookHelpers.ts:70-83` Stop hook + `stopHooks.ts:257-267` blocking → userMessage 重路由）
  - 触发信号：检测主 agent 是否调过 `invoke_skill('test-case-review')`
...
- SKILL.md 重写：
  - frontmatter 采用 skill 标准（name / description / when_to_use 含 Trigger + SKIP / allowed-tools / context: inline）
  - Step 0 写 todo（必做，仿 TodoWriteTool 反偷懒）
  - Steps 每步 Success criteria + 必要的 Execution / Rules / Artifacts（skill 模板）
  - Phase 限定 ONLY path（桶隔离：product/ vs qa/，禁止从 qa/Test List_*.md 推产品定义）
  - 删除"全局约束"段（与主 agent prompt 重复，违反 "通用约束写 system prompt / SKILL 写工作流" 哲学）
  - Step 8 简化为 4 句话："verifier 输出是给用户的最终报告，relay"
  - Step 9 多 sheet xlsx：纯 prompt 路线（仿 simplify Phase 2 "Launch Three Review Agents in Parallel"）
- 废弃 `qa_sanity_check` 工具（不用机械扫描工具；verifier 自发 grep 探索字面问题）
- memory write 端治理：`review_finalizer` 改为返回 None，评审结论不入 memory（不存评审结论的源头治理）
- 一次性脚本 `scripts/maintenance/archive_review_findings.py`：archive 历史 reviews/cases + reviews/tickets 到 archive/ 子目录

### 工程基础设施

- `_sandbox.py` 模块：集中 CWD 解析 + 多根校验（allWorkingDirectories + pwd 模式）；exec_tools.py 路径展开为绝对路径避免 cwd 切换问题
...
- `docs/skill_authoring_standard.md`：项目级 SKILL 编写标准
